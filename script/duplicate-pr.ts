...
import { createJyycode } from "@jyycode-ai/sdk"
...
`)
  ...
  const jyycode = await createJyycode({ port: 0 })
  ...
  try {
    ...
    const session = await jyycode.client.session.create()
    const result = await jyycode.client.session
      .prompt({
