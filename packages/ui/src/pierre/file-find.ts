...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("jyycode-find")
  api.delete("jyycode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("jyycode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("jyycode-find", new Highlight(...rest))
    return true
  }
