...
```typescript
import { InteractionStateName, PivotSheet, S2Event, S2Options, SpreadSheet } from "@antv/s2"
...
s2.interaction.updateDataCellRelevantHeaderCells(InteractionStateName.HOVER, dataCellViewMeta)

// Hide specific columns by node ID
s2.interaction.hideColumns(["root[&]Furniture[&]Table[&]number", "root[&]Stationery[&]Pen[&]number"])
