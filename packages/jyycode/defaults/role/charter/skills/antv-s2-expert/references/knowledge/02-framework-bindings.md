...
const App = () => <SheetComponent sheetType="pivot" dataCfg={s2DataConfig} options={{ width: 600, height: 400 }} />
```
...
```vue
<template>
  <SheetComponent :sheetType="'table'" :dataCfg="s2DataConfig" :options="s2Options" />
</template>
```
...
```vue
<template>
  <SheetComponent :dataCfg="dataCfg" :options="options" @spreadsheet="createSpreadSheet" />
</template>
