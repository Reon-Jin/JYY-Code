...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.JYYCODE_REPO_CLONE_GITHUB_BASE_URL
      process.env.JYYCODE_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.JYYCODE_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.JYYCODE_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("reference", () => {
  it.live("resolves supported local and git config forms", () =>
    Effect.gen(function* () {
      const root = path.resolve("jyycode-reference-root")
    ...
    provideTmpdirInstance(
      (_dir) =>
        Effect.gen(function* () {
          ...
          const cache = path.join(Global.Path.repos, "github.com", "jyycode-reference-test", "repo")
          ...
          const remoteDir = path.join(remoteRoot, "jyycode-reference-test")
        ...
        }),
      {
        config: {
          reference: {
            docs: "jyycode-reference-test/repo",
          },
        },
      },
    ),
  ...
  scout.live("refreshes configured git references on new instance init", () =>
    Effect.gen(function* () {
      ...
      const source = yield* tmpdirScoped({ git: true })
      const remoteRoot = yield* tmpdirScoped()
      const remoteDir = path.join(remoteRoot, "jyycode-reference-refresh")
      const remoteRepo = path.join(remoteDir, "repo.git")
      ...
      yield* githubBase(
        ...
        provideTmpdirInstance(
            ...
            }),
          {
            config: {
              reference: {
                docs: "jyycode-reference-refresh/repo",
              },
            },
          },
        ),
