...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/jyycode")) {
...
const managedConfigDir = process.env.JYYCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "jyycode.json") {
...
async function writeConfig(dir: string, config: object, name = "jyycode.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "jyycode.json") =>
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "jyycode.jsonc"))
    expect(content).toContain('"$schema": "https://jyycode.ai/config.json"')
  } finally {
    ;(Global.Path as { config: string }).config = prev
    await clear(true)
  }
...
test("does not create global config when JYYCODE_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.JYYCODE_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.JYYCODE_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "jyycode.jsonc"))).toBe(false)
  } finally {
    ;(Global.Path as { config: string }).config = prevConfig
    if (prevEnv === undefined) delete process.env.JYYCODE_CONFIG_DIR
    else process.env.JYYCODE_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://jyycode.ai/config.json", shell: "bash" },
      "config.json",
    )
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "jyycode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "jyycode.jsonc")
...
it.instance("ignores legacy tui keys in jyycode config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.promise(() =>
      Filesystem.write(
        path.join(test.directory, "jyycode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://jyycode.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      const content = yield* Effect.promise(() => Filesystem.readText(path.join(test.directory, "jyycode.json")))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["JYYCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { jyycode: { options: { apiKey: "{env:JYYCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["jyycode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["JYYCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["JYYCODE_CONSOLE_TOKEN"]
    }
  }
})
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* Effect.promise(() => Filesystem.write(path.join(test.directory, "jyycode.json"), "{ invalid json }"))
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .jyycode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .jyycode/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "agents", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .jyycode/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "command", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .jyycode/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "commands", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
test("does not try to install dependencies in read-only JYYCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.JYYCODE_CONFIG_DIR
  process.env.JYYCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.JYYCODE_CONFIG_DIR
    else process.env.JYYCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable JYYCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .jyycode config
      const projectDir = path.join(dir, "project")
      const jyycodeDir = path.join(projectDir, ".jyycode")
      await fs.mkdir(jyycodeDir, { recursive: true })
      ...
      // Local .jyycode config with different plugins
      await Filesystem.write(
        path.join(jyycodeDir, "jyycode.json"),
        JSON.stringify({
          $schema: "https://jyycode.ai/config.json",
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".jyycode", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".jyycode", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .jyycode config with some overlapping plugins
      await Filesystem.write(
        path.join(jyycodeDir, "jyycode.json"),
        JSON.stringify({
          $schema: "https://jyycode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".jyycode")
...
// Note: preload.ts sets JYYCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("local .jyycode config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://jyycode.ai/config.json",
    ...
    yield* mkdirEffect(path.join(test.directory, ".jyycode"))
    yield* writeConfigEffect(
      path.join(test.directory, ".jyycode"),
      {
        $schema: "https://jyycode.ai/config.json",
        mcp: {
          docs: {
  ...
  await provideTmpdirInstance(
    () =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          const config = yield* svc.get()
          expect(seen.wellKnown).toBe("https://example.com/.well-known/jyycode")
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/jyycode`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
test("wellknown remote_config supports templated env vars in headers", async () => {
  ...
  const client = remoteConfigClient({
    ...
    wellKnown: {
      remote_config: {
        url: "https://config.example.com/jyycode.json",
        headers: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(seen.wellKnown).toBe("https://example.com/.well-known/jyycode")
            expect(seen.remote).toBe("https://config.example.com/jyycode.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "jyycode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-jyycode@2.4.3", file)).toBe("oh-my-jyycode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-jyycode@2.4.3", "file:///project/.jyycode/plugin/oh-my-jyycode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.jyycode/plugin/demo.ts", "file:///project/.jyycode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.jyycode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const jyycodeDir = path.join(projectDir, ".jyycode")
        const pluginDir = path.join(jyycodeDir, "plugin")
...
describe("JYYCODE_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
    "skips project config files when flag is set",
    () =>
      withProcessEnv(
        "JYYCODE_DISABLE_PROJECT_CONFIG",
        "true",
        Effect.gen(function* () {
          const config = yield* Config.use.get()
  ...
  it.instance("skips project .jyycode/ directories when flag is set", () =>
    withProcessEnv(
      "JYYCODE_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* mkdirEffect(path.join(test.directory, ".jyycode", "command"))
        yield* writeTextEffect(
          path.join(test.directory, ".jyycode", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { JYYCODE_CONFIG_DIR: undefined, JYYCODE_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "JYYCODE_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { JYYCODE_DISABLE_PROJECT_CONFIG: "true", JYYCODE_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed JYYCODE_PERMISSION JSON used to crash
...
describe("JYYCODE_PERMISSION env var", () => {
  it.instance("does not crash when JYYCODE_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "JYYCODE_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("JYYCODE_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in JYYCODE_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in JYYCODE_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "JYYCode Managed",
          PayloadIdentifier: "ai.jyycode.managed.test",
          PayloadType: "ai.jyycode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://jyycode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://jyycode.ai/config.json")
})
