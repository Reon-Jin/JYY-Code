 ...
 *
 * The script intentionally isolates `JYYCODE_DB` before importing modules that touch
...
void (await import("@jyycode-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "jyycode.jsonc"),
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-jyycode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-jyycode-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "jyycode", model: "test" })}`,
      headers: ctx.headers(),
    }))
