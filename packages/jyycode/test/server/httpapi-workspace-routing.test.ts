...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      // This starts a second HTTP server that stands in for the jyycode server
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          ...
          "x-jyycode-directory": "/secret/path",
          "x-jyycode-workspace": "internal",
        }),
        HttpClient.execute,
      )
      ...
      expect(forwarded?.headers["x-jyycode-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-jyycode-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-jyycode-directory", headerDir),
        HttpClient.execute,
      )
