...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips jyycode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-jyycode-directory"]).toBeUndefined()
      expect(forwarded["x-jyycode-workspace"]).toBeUndefined()
