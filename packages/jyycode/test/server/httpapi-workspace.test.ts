    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://jyycode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-jyycode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-jyycode-workspace")
      } finally {
