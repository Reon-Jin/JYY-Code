...
type Sdk = ReturnType<typeof createJyycodeClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.JYYCODE_SERVER_PASSWORD = input?.password
  Flag.JYYCODE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".jyycode", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("jyycode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("jyycode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
