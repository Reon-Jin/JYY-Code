// Subprocess test harness for the jyycode CLI. Spawns the real binary against
...
//
// Configuration flows through jyycode's built-in test affordances:
//   - JYYCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - JYYCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - JYYCODE_DISABLE_PROJECT_CONFIG : skip walking up for jyycode.json
//   - JYYCODE_PURE                : skip external plugin discovery + install
//   - JYYCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `jyycode.run` is fully wired. The shape supports adding more
// builders (`jyycode.serve(opts)`, `jyycode.acp(opts)`, `jyycode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of JyycodeCli.
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    JYYCODE_CONFIG_CONTENT: configJson,
    JYYCODE_DISABLE_PROJECT_CONFIG: "1",
    JYYCODE_PURE: "1",
    JYYCODE_DISABLE_AUTOUPDATE: "1",
    JYYCODE_DISABLE_AUTOCOMPACT: "1",
...
// Typed equivalent of constructing argv for `jyycode run`. New flags should
...
// `jyycode serve` is a long-lived process — it never exits on its own.
...
export type JyycodeCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `jyycode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `jyycode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("jyycode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("jyycode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `jyycode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("jyycode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const jyycode: JyycodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, jyycode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `jyycode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "jyycode") {
...
// every call site. Use this for any test that needs the jyycode CLI fixture.
//
...
// resources (e.g. `jyycode.serve`) without an extra `Effect.scoped` wrapper —
