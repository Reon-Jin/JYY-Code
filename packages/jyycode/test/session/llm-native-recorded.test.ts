...
import { ModelsDev } from "@jyycode-ai/core/models-dev"
import { HttpRecorder, Redactor } from "@jyycode-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@jyycode-ai/llm"
...
const zenURL = (connection: string) => `https://console.jyycode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.JYYCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["jyycode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("JYYCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("JYYCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["jyycode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["jyycode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.JYYCODE_RECORD_CONSOLE_TOKEN && process.env.JYYCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.jyycode,
        name: "JYYCode Zen",
        env: ["JYYCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.JYYCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.JYYCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.JYYCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["jyycode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("JYYCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("JYYCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("JYYCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set JYYCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the jyycode LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "jyycode.json"),
      JSON.stringify({ $schema: "https://jyycode.ai/config.json", ...scenario.config(model) }),
    ),
  )
