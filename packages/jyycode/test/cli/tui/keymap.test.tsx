...
test("email session shortcuts use ctrl arrow keys", async () => {
  const sequences: Record<string, string[][]> = {}
  ...
  function Harness() {
    ...
    const config = createTuiResolvedConfig()
    const offKeymap = registerJyycodeKeymap(keymap, renderer, config)
    const offSessionLayer = keymap.registerLayer({
      bindings: config.keybinds.gather("app", ["session.email.view", "session.email.back"]),
    })
    const bindings = keymap.getCommandBindings({
      visibility: "registered",
      commands: ["session.email.view", "session.email.back"],
    })
    sequences.emailView = bindings.get("session.email.view")?.map((binding) => keySequence(binding)) ?? []
    sequences.emailBack = bindings.get("session.email.back")?.map((binding) => keySequence(binding)) ?? []
    onCleanup(() => {
      offSessionLayer()
      offKeymap()
    })
  ...
  const app = await testRender(() => <Harness />)
  try {
    expect(sequences.emailView).toEqual([["ctrl+right"]])
    expect(sequences.emailBack).toEqual([["ctrl+left"]])
  } finally {
    app.renderer.destroy()
  }
})
