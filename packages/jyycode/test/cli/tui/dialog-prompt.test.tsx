...
async function mountPrompt(input: {
  ...
  const { Global } = await import("@jyycode-ai/core/global")
  const previous = {
  ...
  const [
    ...
    { TuiConfigProvider },
    { ToastProvider },
    { JyycodeKeymapProvider, registerJyycodeKeymap },
  ] = await Promise.all([
  ...
  function Harness() {
    ...
    const off = registerJyycodeKeymap(keymap, renderer, resolvedConfig)
    onCleanup(off)
