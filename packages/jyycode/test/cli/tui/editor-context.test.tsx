...
const originalJyycodePort = process.env.JYYCODE_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.JYYCODE_EDITOR_SSE_PORT = originalJyycodePort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.JYYCODE_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with JYYCODE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.JYYCODE_EDITOR_SSE_PORT = "4020"
