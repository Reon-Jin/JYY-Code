...
test.skipIf(!symlinkAvailable)("resolveZedDbPath skips candidates that cannot be stated", async () => {
  await using tmp = await tmpdir()
  const loop = path.join(tmp.path, "loop")
  await symlink(loop, loop)
  const home = spyOn(os, "homedir").mockImplementation(() => tmp.path)
  const previous = process.env.JYYCODE_ZED_DB
  process.env.JYYCODE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.JYYCODE_ZED_DB
    else process.env.JYYCODE_ZED_DB = previous
    home.mockRestore()
  }
})
