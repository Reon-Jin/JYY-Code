// Subprocess integration tests for `jyycode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `jyycode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `JYYCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { cliIt } from "../../lib/cli-process"

describe("jyycode run (non-interactive subprocess)", () => {
  ...
  // (the prompt was accepted, then the upstream provider failed), jyycode
  ...
  cliIt.concurrent(
    ...
    ({ llm, jyycode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* jyycode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, jyycode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* jyycode.run("say hi", { format: "json" })
        jyycode.expectExit(result, 0)

        const events = jyycode.parseJsonEvents(result.stdout)
