...
import { JyycodeClient, type GlobalEvent } from "@jyycode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<JyycodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<JyycodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<JyycodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<JyycodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<JyycodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://jyycode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: JyycodeClient["event"]["subscribe"]
    globalEvent?: JyycodeClient["global"]["event"]
    promptAsync?: JyycodeClient["session"]["promptAsync"]
    status?: JyycodeClient["session"]["status"]
    messages?: JyycodeClient["session"]["messages"]
    children?: JyycodeClient["session"]["children"]
    permissions?: JyycodeClient["permission"]["list"]
    questions?: JyycodeClient["question"]["list"]
  } = {},
) {
  const client = new JyycodeClient()

  const subscribe: JyycodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: JyycodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: JyycodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: JyycodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: JyycodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: JyycodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: JyycodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: JyycodeClient["question"]["list"] = input.questions ?? (() => ok([]))
