...
describe("jyycode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // JYYCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["mcp", "list"])
        jyycode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["providers", "list"])
        jyycode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["models"])
        jyycode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["agent", "list"])
        jyycode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh JYYCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["session", "list"])
        jyycode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["stats"])
        jyycode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under JYYCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ jyycode }) =>
      Effect.gen(function* () {
        const r = yield* jyycode.spawn(["db", "path"])
        jyycode.expectExit(r, 0, "db path")
