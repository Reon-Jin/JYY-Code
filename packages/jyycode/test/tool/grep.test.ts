  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "jyycode-grep-alias-"))),
  ...
  gitRefs.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cache = path.join(Global.Path.repos, "github.com", "jyycode-grep-reference", "repo")
        ...
        const remoteDir = path.join(remoteRoot, "jyycode-grep-reference")
      ...
      }),
    {
      config: {
        reference: {
          docs: "jyycode-grep-reference/repo",
        },
      },
    },
  )
