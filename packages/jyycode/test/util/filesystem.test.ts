...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "jyycode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "jyycode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jyycode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "jyycode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["jyycode.json", "jyycode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "jyycode.json"),
        path.join(tmp.path, "jyycode.jsonc"),
        path.join(project, "jyycode.json"),
        path.join(project, "jyycode.jsonc"),
      ])
    })
  })
