...
describe("file-backed plan protocol", () => {
  ...
  it("derives system prompts by session mode", () => {
    ...
    expect(multiAgentPrompt).toContain("# 主 Agent 多智能体协议")
    ...
    expect(multiAgentPrompt).toContain("Candidate 只用于比较")
    expect(multiAgentPrompt).toContain("standard reject 会自动清理工作区并按原 role 启动新 run")
    expect(multiAgentPrompt).toContain("不要再次 Dispatch_dispatch")
    expect(multiAgentPrompt).toContain("Merge_apply({task_id})")
    expect(multiAgentPrompt).toContain("Blackboard 是主/子 Agent 的协作通道")
    ...
    expect(childPrompt).toContain("Standard child 开始和被唤醒时先无参调用 Blackboard")
