...
describe("startup plan workspace cleanup", () => {
  it("removes orphaned child workspaces and merge journals", () => {
    ...
    const staleLease = path.join(runtimeRoot, "jyycode-ses_old-s1_t1-0123456789ab.lease.json")
    ...
    fs.writeFileSync(staleLease, "{}")

    const result = cleanupStartupPlanWorkspaces({ runtimeRoot, workspaceRoots: [workspaceRoot] })

    expect(result.removed.sort()).toEqual([staleBaseline, staleChild, staleJournal, staleManifest, staleLease].sort())
  ...
  it("preserves workspaces and journals referenced by active plans", () => {
    ...
    const lease = path.join(runtimeRoot, "jyycode-ses_main-s1_t1-0123456789ab.lease.json")
    ...
    expect(result.preserved.sort()).toEqual([baseline, child, journal, manifest, lease].sort())
