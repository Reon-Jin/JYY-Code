...
const PORT = 41992
const dir = mkdtempSync(join(tmpdir(), "jyy-tmp2-"))
...
const iv = setInterval(() => void check().catch(() => {}), 4000)
setTimeout(() => { clearInterval(iv); try { server.kill() } catch {}; try { tui?.kill() } catch {}; process.exit(0) }, 30000)
