...
- Author package entrypoint is `@jyycode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://jyycode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/jyycode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@jyycode-ai/plugin/tui`.
- `@jyycode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running JYYCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.jyycode` is absent, no check is performed (backward compatible).
...
- `jyycode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `jyycode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.jyycode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.jyycode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.jyycode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.jyycode` dir or `JYYCODE_CONFIG_DIR`, JYYCode installs `@jyycode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@jyycode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus JYYCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
JYYCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, JYYCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@jyycode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"jyycode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `jyycode.default`.
...
- Local plugins persist installed themes under the local `.jyycode/themes` area near the plugin config source.
...
- `--pure` / `JYYCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.jyycode/plugins/tui-smoke.tsx`
- Local vim plugin: `.jyycode/plugins/tui-vim.tsx`
- Local smoke config: `.jyycode/tui.json`
- Local smoke theme: `.jyycode/plugins/smoke-theme.json`
