...
Practical reference for a future `packages/server` split after the jyycode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/jyycode`.
...
Until enough shared service code lives outside `packages/jyycode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/jyycode`

It should not import `packages/jyycode` services while `packages/jyycode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on jyycode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/jyycode` implementation details.
