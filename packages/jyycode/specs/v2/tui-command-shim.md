...
Delete `packages/jyycode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/jyycode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/jyycode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/jyycode`
- TUI plugin loader tests in `packages/jyycode` if runtime plugin API wiring changed
