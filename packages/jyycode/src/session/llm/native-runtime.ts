...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("jyycode"))
    return { type: "unsupported", reason: "provider is not openai, jyycode, or anthropic" }
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains jyycode-owned. The native runtime only adapts
      // the @jyycode-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
