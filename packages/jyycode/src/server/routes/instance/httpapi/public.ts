...
import { JYYCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for jyycode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(JYYCodeHttpApi)
...
export const PublicApi = JYYCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "jyycode",
    version: "1.0.0",
    description: "jyycode api",
