...
import { LocationServiceMap } from "@jyycode-ai/core/location-layer"
import { PluginBoot } from "@jyycode-ai/core/plugin/boot"
...
>()("@jyycode/ExperimentalHttpApiV2Location") {}

function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  const query = new URL(request.url, "http://localhost").searchParams
  return {
    directory: query.get("location[directory]") || request.headers["x-jyycode-directory"] || process.cwd(),
    workspaceID: query.get("location[workspace]") || request.headers["x-jyycode-workspace"],
  }
}
