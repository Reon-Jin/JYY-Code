...
import { StorageCommand as StorageMaintenanceCommand } from "./cli/cmd/storage"
...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.JYYCODE = "1"
    process.env.JYYCODE_PID = String(process.pid)

    Log.Default.info("jyycode", {
    ...
    if (!(await Filesystem.exists(marker))) {
      ...
      try {
        await JsonMigration.run(drizzle({ client: Database.legacyClient().$client }), {
          progress: (event) => {
    ...
    try {
      ...
      Log.Default.warn("plan workspace sweeper skipped", {
