// EventV2 persistence and notification adapter. EventV2 remains the product
// event API; this adapter is the static kernel boundary that stores versioned
// events and publishes compatibility notifications for older clients.
...
import "@jyycode-ai/core/catalog"
...
export class Service extends Context.Service<Service, EventV2.Interface>()("@jyycode/EventRuntime") {}
      ...
      Effect.gen(function* () {
        ...
        return yield* store.load({ directory: event.location.directory }).pipe(
          Effect.flatMap((loaded) => {
            const withInstance = effect.pipe(Effect.provideService(InstanceRef, loaded))
