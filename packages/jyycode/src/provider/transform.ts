...
import type * as ModelsDev from "@jyycode-ai/core/models-dev"
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "jyycode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("jyycode")) {
      result["promptCacheKey"] = input.sessionID
