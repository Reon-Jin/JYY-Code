...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "jyycode.agent.name": input.agent,
      "jyycode.directory": input.directory,
      "jyycode.first": input.first,
      "jyycode.model.provider": input.model?.providerID,
      "jyycode.model.id": input.model?.modelID,
      "jyycode.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "jyycode.show_exit": next.showExit,
