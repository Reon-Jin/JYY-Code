...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "jyycode.mode": input.resolveSession ? "local" : "attach",
      "jyycode.initial_input": !!input.initialInput,
      "jyycode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "jyycode.directory": ctx.directory,
        "jyycode.resume": ctx.resume === true,
        "jyycode.agent.name": state.agent,
        "jyycode.model.provider": state.model?.providerID,
        "jyycode.model.id": state.model?.modelID,
        "jyycode.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "jyycode.model.provider": model.providerID,
            "jyycode.model.id": model.modelID,
            "jyycode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "jyycode.model.id": state.model?.modelID,
                "jyycode.model.variant": state.activeVariant,
                "jyycode.prompt.chars": prompt.text.length,
                "jyycode.prompt.parts": prompt.parts.length,
                "jyycode.prompt.include_files": includeFiles,
                "jyycode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "jyycode.directory": input.directory,
      "jyycode.initial_input": !!input.initialInput,
      "jyycode.demo": input.demo,
    },
    async () => {
