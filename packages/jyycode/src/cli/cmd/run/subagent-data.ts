import type { Event, Message, Part, PermissionRequest, QuestionRequest, ToolPart } from "@jyycode-ai/sdk/v2"
...
function taskTab(part: ToolPart, sessionID: string): FooterSubagentTab {
  ...
  const modelLabel =
    model && typeof model === "object"
      ? [text(Reflect.get(model, "providerID")), text(Reflect.get(model, "modelID"))].filter(Boolean).join("/")
      : text(part.state.input.model)
