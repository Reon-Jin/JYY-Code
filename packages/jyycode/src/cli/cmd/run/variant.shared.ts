...
//
// The saved variant persists across sessions in ~/.local/state/jyycode/model.json
...
class Service extends Context.Service<Service, VariantService>()("@jyycode/RunVariant") {}
...
function modelInfo(providers: RunProvider[] | undefined, model: NonNullable<RunInput["model"]>) {
  ...
  const displayName = provider?.models[model.modelID]?.name
  const modelName = displayName && displayName !== model.modelID ? `${displayName} (${model.modelID})` : model.modelID
  return {
