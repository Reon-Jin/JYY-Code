...
import type { JyycodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@jyycode-ai/sdk/v2"
...
type PromptModel = Parameters<JyycodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<JyycodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<JyycodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<JyycodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<JyycodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<JyycodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<JyycodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<JyycodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<JyycodeClient["question"]["reject"]>[0]
