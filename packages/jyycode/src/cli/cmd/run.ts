// CLI entry point for `jyycode run`.
//
// Handles three modes:
...
//      jyycode server and runs interactive mode against it.
//
...
type ModelInput = Parameters<JyycodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run jyycode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running jyycode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to JYYCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to JYYCODE_SERVER_USERNAME or 'jyycode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const root = Filesystem.resolve(process.env.JYYCODE_PWD ?? process.env.PWD ?? process.cwd())
      const directory = (() => {
      ...
      async function session(sdk: JyycodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: JyycodeClient, sessionID: string) {
      ...
      async function current(sdk: JyycodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: JyycodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: JyycodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: JyycodeClient) {
        ...
        async function loop(client: JyycodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createJyycodeClient({
        baseUrl: "http://jyycode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
