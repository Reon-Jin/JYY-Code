...
export const JYYCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const JYYCODE_MODE_KEY = "jyycode.mode"

export const JyycodeKeymapProvider = KeymapProvider
export const useJyycodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type JyycodeModeStack = ReturnType<typeof createJyycodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, JyycodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createJyycodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(JYYCODE_MODE_KEY, JYYCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(JYYCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(JYYCODE_MODE_KEY, stack.at(-1)?.mode ?? JYYCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? JYYCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(JYYCODE_MODE_KEY, undefined)
...
export function useJyycodeModeStack() {
  return getJyycodeModeStack(useJyycodeKeymap())
}

export function getJyycodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Jyycode mode stack is not registered for this keymap")
  return value
}
...
export function registerJyycodeKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createJyycodeModeStack(keymap)
