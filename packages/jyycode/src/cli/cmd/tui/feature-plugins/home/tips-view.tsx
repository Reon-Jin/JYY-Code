...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at jyycode.ai",
  ...
  "Create {highlight}jyycode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/jyycode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.jyycode/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.jyycode/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.jyycode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.jyycode/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent JYYCode from reading sensitive files",
  "Use {highlight}jyycode run{/highlight} for non-interactive scripting",
  "Use {highlight}jyycode --continue{/highlight} to resume the last session",
  "Use {highlight}jyycode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}jyycode serve{/highlight} for headless API access to JYYCode",
  "Use {highlight}jyycode run --attach{/highlight} to connect to a running server",
  "Run {highlight}jyycode upgrade{/highlight} to update to the latest version",
  "Run {highlight}jyycode auth list{/highlight} to see all configured providers",
  "Run {highlight}jyycode agent create{/highlight} for guided agent creation",
  "Use {highlight}/jyycode{/highlight} in GitHub issues/PRs to trigger AI actions",
  ...
  "Run {highlight}jyycode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/jyycode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with JYYCode Zen for curated, tested models",
