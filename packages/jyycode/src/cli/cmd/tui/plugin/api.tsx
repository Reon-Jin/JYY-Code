import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@jyycode-ai/plugin/tui"
...
import type { useJyycodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useJyycodeKeymap>
...
function stateApi(sync: ReturnType<typeof useSync>): TuiPluginApi["state"] {
  return {
    ...
    session: {
      ...
      context(sessionID) {
        return sync.data.session_context[sessionID]
      },
      status(sessionID) {
      ...
      agentCluster(sessionID) {
        return sync.data.agent_cluster[sessionID] ?? { runs: [], tasks: [] }
      },
    },
    part(messageID) {
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getJyycodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getJyycodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
