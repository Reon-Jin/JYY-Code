...
const TUI_SCHEMA_URL = "https://jyycode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from jyycode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const jyycode = await jyycodeFiles(input)
  for (const file of jyycode) {
...
async function jyycodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "jyycode"),
    ...(await Filesystem.findUp(["jyycode.json", "jyycode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "jyycode"))
  }
  if (Flag.JYYCODE_CONFIG) files.push(Flag.JYYCODE_CONFIG)
