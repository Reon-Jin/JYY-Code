...
const GO_UPSELL_PROVIDERS = new Set(["jyycode", "jyycode-go"])
...
export function Session() {
  ...
  // Show prompt for child sessions too — users need to guide subagents
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}jyycode -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
        ...
        await sdk.client.session
          ...
          .then(() => {
            ...
            toast.show({
              message:
                status?.type === "idle"
                  ? `Multi-Agent ${next ? "enabled" : "disabled"}`
                  : `Multi-Agent ${next ? "enabled" : "disabled"} for the next prompt`,
              variant: "info",
              duration: 2500,
            })
          })
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  // OpenAI / Copilot / jyycode-via-OpenAI emit `**Title**\n\n<body>` summary
