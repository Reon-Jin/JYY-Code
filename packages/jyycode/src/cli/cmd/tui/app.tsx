...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.JYYCODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
export function tui(input: {
  ...
  return new Promise<void>(async (resolve) => {
    ...
    const offKeymap = registerJyycodeKeymap(keymap, renderer, input.config)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.JYYCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.JYYCODE_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("JYYCode")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`JYYCode | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`JYYCode | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://jyycode.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to JYYCode v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.JYYCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
