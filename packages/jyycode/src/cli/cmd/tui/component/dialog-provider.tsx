...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@jyycode-ai/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__jyycode_custom_provider__"
    ...
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          ...
          "jyycode-go": "Low cost subscription for everyone",
        }[provider.id],
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in jyycode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          ...
          "jyycode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                JYYCode Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://jyycode.ai/zen</span> and enable JYYCode Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in jyycode.json to use it.`,
          })
          dialog.clear()
          return
        }
