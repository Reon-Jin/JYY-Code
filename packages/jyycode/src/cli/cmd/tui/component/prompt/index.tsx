...
import type { AssistantMessage, FilePart, UserMessage } from "@jyycode-ai/sdk/v2"
...
export function Prompt(props: PromptProps) {
  ...
  const usage = createMemo(() => {
    ...
    const estimate = sync.data.session_context[props.sessionID]
    ...
    if (!formatted.provider && !formatted.estimated && cost <= 0) return
    return {
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.JYYCODE_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
                    ...
                    <Match when={usage()}>
                      {(item) => (
                        <text fg={theme.textMuted} wrapMode="none">
                          {[item().provider, item().estimated, item().cost].filter(Boolean).join(" · ")}
                        </text>
                      )}
                    </Match>
                    <Match when={true}>
