...
const GCCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      .option("root", { type: "string", description: "storage root override for diagnostics/tests" })
      .option("dry-run", { type: "boolean", default: true, description: "report collectable files without deleting them" })
      .option("json", { type: "boolean", default: false, description: "write machine-readable JSON" }),
...
const MaintainCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("dry-run", { type: "boolean", default: true, description: "report maintenance without modifying the database" })
      .option("full", { type: "boolean", default: false, description: "explicitly request offline VACUUM INTO and atomic replacement" })
      .option("json", { type: "boolean", default: false, description: "write machine-readable JSON" }),
