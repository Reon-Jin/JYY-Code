...
export const DEPRECATED_PLUGIN_PACKAGES = ["jyycode-openai-codex-auth", "jyycode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, jyycodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(jyycodeVersion) || semver.major(jyycodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.jyycode
  if (typeof range !== "string") return
  if (!semver.satisfies(jyycodeVersion, range)) {
    throw new Error(`Plugin requires jyycode ${range} but running ${jyycodeVersion}`)
  }
}
