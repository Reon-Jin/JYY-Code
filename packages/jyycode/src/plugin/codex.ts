...
<html>
  <head>
    <title>JYYCode - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>JYYCode - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              ...
              return match ? Number(match[1]) > 5.4 : false
            })
            .map(([modelID, model]) => [
              modelID,
              {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "jyycode"
      output.headers["User-Agent"] = `jyycode/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
