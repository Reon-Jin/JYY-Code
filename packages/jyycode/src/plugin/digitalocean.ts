import type { Hooks, PluginInput } from "@jyycode-ai/plugin"
import type { Model } from "@jyycode-ai/sdk/v2"
...
const MAK_NAME_PREFIX = "jyycode-oauth"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>JYYCode - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to JYYCode."
...
async function createModelAccessKey(bearer: string): Promise<ApiKeyInfo> {
  ...
  const res = await fetch(`${DO_API_BASE}/v2/gen-ai/models/api_keys`, {
    ...
    headers: {
      ...
      "User-Agent": `jyycode/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              ...
              instructions:
                "Sign in to DigitalOcean in your browser. JYYCode will create a Model Access Key named jyycode-oauth-* and load your Inference Routers. Re-run /connect to refresh routers later.",
