...
def main() -> int:
    """List presets without opening a GUI, unless an explicit example is requested."""
    parser = argparse.ArgumentParser(description="Matplotlib publication style presets.")
    parser.add_argument("--list", action="store_true", help="list styles and palettes")
    args = parser.parse_args()
    if not args.list:
        parser.print_help()
        return 0

    print("Publication styles: default, nature, science, cell, minimal, presentation")
    print("Color palettes: okabe_ito, wong, tol_bright, tol_muted, tol_high_contrast")
    return 0
