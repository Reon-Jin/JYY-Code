...
def configure_console_encoding() -> None:
    """Use UTF-8 when possible so Windows legacy consoles do not reject output."""
...
def comprehensive_assumption_check(
    ...
    if group_col is not None:
        ...
        normality_results = check_normality_per_group(
            data, value_col, group_col, alpha=alpha, plot=plot
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run statistical assumption checks.")
    parser.add_argument(
        "--example",
        action="store_true",
        help="run a deterministic in-memory example without opening plots",
    )
    parser.add_argument(
        "--show-plots",
        action="store_true",
        help="show plots for the --example run when the active host supports a display",
    )
    ...
    dataframe = pd.DataFrame(
        {
            "score": np.concatenate([np.random.normal(75, 8, 50), np.random.normal(68, 10, 50)]),
            "group": ["A"] * 50 + ["B"] * 50,
        },
    )
