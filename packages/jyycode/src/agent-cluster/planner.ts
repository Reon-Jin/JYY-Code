export const ClusterPrimaryPrompt = [
  ...
  "   ```",
  "4. For revision_requested decisions: call the task tool again with the SAME task_id and the revision_prompt as the new prompt. This resumes the same subagent session so it can fix its work incrementally.",
  "5. For failed decisions: mark the task failed, note the risks, and if warranted create a replacement task (different task_id) to recover the work.",
...
export function runInstructions(input: {
  ...
  return [
    ...
    "```",
    `Respect max_review_rounds=${input.maxReviewRounds}. Use the same task_id for revisions.`,
    "",
    "Model routing:",
