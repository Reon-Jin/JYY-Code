export const ClusterPrimaryPrompt = [
  ...
  "",
  "CRITICAL: Before dispatching ANY subagents, you MUST first output the COMPLETE plan as a ```json code block. The JSON must contain \"goal\" and \"tasks\" with full details. The runtime reads this JSON to persist tasks to the database — without it, every task dispatch will fail.",
  "",
  "Your FIRST assistant response MUST follow this exact order:",
  "1. Brief greeting",
  "2. ```json code block (complete plan)",
  "3. Natural language summary",
  "4. task tool calls for step-1 tasks (same response, immediately after the summary)",
  "",
  "Do NOT:",
  "- Output a markdown plan without a JSON block",
  "- Output JSON after dispatching tasks",
  "- Output JSON in a later response",
  "",
...
export function runInstructions(input: {
  ...
  return [
    ...
    "",
    "CRITICAL: Before ANY task tool call, you MUST output the COMPLETE plan as a ```json code block WITH the exact JSON plan containing \"goal\" and \"tasks\". The runtime extracts this JSON to persist plan rows; without it dispatch will fail 30 times.",
    "",
    "CORRECT ordering in your FIRST response:",
    "1. Brief greeting",
    "2. ```json code block with the full plan (goal + tasks array)",
