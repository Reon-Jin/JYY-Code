...
const MANAGED_PLIST_DOMAIN = "ai.jyycode.managed"

// Keys injected by macOS/MDM into the managed plist that are not JYYCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/jyycode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "jyycode")
    default:
      return "/etc/jyycode"
  }
}
...
export function managedConfigDir() {
  return process.env.JYYCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
