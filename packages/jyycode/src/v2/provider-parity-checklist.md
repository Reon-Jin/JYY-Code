...
This tracks legacy provider behavior from `packages/jyycode/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/jyycode/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.
...
- [x] Jyycode unauthenticated paid-model filtering.
- [x] Jyycode public API key fallback.
...
- [x] Jyycode small model priority. Replaced by cheapest output cost selection.
