...
export function init({ sdk: _sdk }: { sdk: JyycodeClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `jyycode auth login` in the terminal",
      name: "Login with jyycode",
      id: "jyycode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // JYYCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const jyycodeProvider = providers.find((p) => p.id === "jyycode")
  if (jyycodeProvider) {
    const [best] = Provider.sort(Object.values(jyycodeProvider.models))
    if (best) {
      return {
