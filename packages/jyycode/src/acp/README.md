...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for jyycode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal jyycode sessions
...
jyycode acp --cwd /path/to/project
```
...
```bash
JYYCODE_ENABLE_QUESTION_TOOL=1 jyycode acp
```
...
- **Terminal Integration**: Full terminal support via jyycode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | jyycode acp
```
...
ACP sessions map cleanly to jyycode's internal session model:
