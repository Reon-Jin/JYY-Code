...
export const TaskStatusTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskStatusTool.execute")(function* (
      ...
      if (!flags.experimentalBackgroundSubagents && _ctx.agent !== "cluster") {
        return yield* Effect.fail(new Error("task_status requires JYYCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true"))
      }
