...
Replace runtime Bun shell template-tag usage in `packages/jyycode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/jyycode/src` is removed except:
