...
const SIDECAR_SERVICE_NAME = "jyycode server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    JYYCODE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    JYYCODE_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`jyycode:${password}`).toString("base64")
