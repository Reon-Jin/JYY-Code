...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.jyycode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `jyycode.settings.dat` is special: it maps to the `jyycode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"jyycode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // jyycode.settings.dat → the electron settings store ("jyycode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "jyycode.global.dat").
  const storeName = filename === "jyycode.settings.dat" ? "jyycode.settings" : filename
