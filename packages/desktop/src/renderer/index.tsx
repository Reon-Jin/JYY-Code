...
import "./styles.css"
import { useTheme } from "@jyycode-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.JYYCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "jyycode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__JYYCODE__ ??= {}
  const pending = window.__JYYCODE__.deepLinks ?? []
  window.__JYYCODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("jyycode.global.dat").getItem("language")
