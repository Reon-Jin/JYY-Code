...
With JYYCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, JYYCode uses our own `jyycode` theme.
...
JYYCode comes with several built-in themes.
...
- Want JYYCode to match their terminal's appearance
...
JYYCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/jyycode/themes/*.json` or `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.jyycode/themes/*.json`
4. **Current working directory** - Defined in `./.jyycode/themes/*.json`
