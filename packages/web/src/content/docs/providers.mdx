---
title: Providers
description: Using any LLM provider in JYYCode.
---

import config from "../../../config.mjs"
export const console = config.console

JYYCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your JYYCode config.
...
in `~/.local/share/jyycode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your JYYCode
config.
...
JYYCode Zen is a list of models provided by the JYYCode team that have been
tested and verified to work well with JYYCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with JYYCode Zen.
:::

1. Run the `/connect` command in the TUI, select `JYYCode Zen`, and head to [jyycode.ai/auth](https://jyycode.ai/zen).
...
It works like any other provider in JYYCode and is completely optional to use.

---

## JYYCode Go

JYYCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the JYYCode team that have been
tested and verified to work well with JYYCode.

1. Run the `/connect` command in the TUI, select `JYYCode Go`, and head to [jyycode.ai/auth](https://jyycode.ai/zen).
...
To use Amazon Bedrock with JYYCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running jyycode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY jyycode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile jyycode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX jyycode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `jyycode.json`:

   ```json title="jyycode.json"
   {
     "$schema": "https://jyycode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
JYYCode. Anthropic explicitly prohibits this.

Previous versions of JYYCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in JYYCode with zero setup:
...
You can configure jyycode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="jyycode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for jyycode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX jyycode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX jyycode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your jyycode config.
...
JYYCode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; JYYCode auto-creates a Model Access Key and discovers your available Models & Inference Routers.
...
3. Your browser opens to authorize JYYCode. Sign in and approve.

   :::note
   JYYCode creates a Model Access Key named `jyycode-oauth-<timestamp>` in your DigitalOcean account. You can rotate or revoke it from the **Model Access Keys** page in the "Manage" section of the DigitalOcean console under Inference.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, JYYCode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — JYYCode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in JYYCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

JYYCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in jyycode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
JYYCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock JYYCode
to only use your own GitLab-hosted instance, add the following to your
`jyycode.json` file. It is also recommended to disable session sharing.
...
Customize through `jyycode.json`:
...
When a `duo-workflow-*` model is selected, JYYCode will:
...
4. Route tool execution requests through JYYCode's permission-gated tool system
...
To use your GitHub Copilot subscription with jyycode:
...
To use Google Vertex AI with JYYCode:
...
2. Set the required environment variables:
   ...
   Set them while running jyycode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id jyycode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through jyycode, you can always configure it yourself.
...
```jsonc title="~/.config/jyycode/jyycode.jsonc"
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [jyycode-helicone-session](https://github.com/H2Shami/jyycode-helicone-session) plugin to automatically log each JYYCode conversation as a session in Helicone.

```bash
npm install -g jyycode-helicone-session
```
...
```json title="jyycode.json"
{
  "plugin": ["jyycode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each JYYCode conversation listed as a separate session.
...
You can configure jyycode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="jyycode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure jyycode to use local models through LM Studio.

```json title="jyycode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure jyycode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for JYYCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/jyycode) for details.
:::

```json title="jyycode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with JYYCode:
...
3. Copy the API key for use in JYYCode.
...
6. **Important**: Before using cloud models in JYYCode, you must pull the model information locally:
...
JYYCode Zen is a list of tested and verified models provided by the JYYCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>JYYCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **JYYCode Zen**.

   ```txt
   /connect
   ```

3. Enter your JYYCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your jyycode config.
...
5. You can also customize them through your jyycode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' jyycode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group jyycode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Jyycode:
...
You can also customize models through your jyycode config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. JYYCode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
JYYCode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when JYYCode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running JYYCode — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. JYYCode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. JYYCode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with jyycode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in jyycode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `jyycode.json` file in your project directory:

   ```json title="jyycode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://jyycode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="jyycode.json" {9,11,17-20}
{
  "$schema": "https://jyycode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow JYYCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `jyycode auth list` to see if the credentials
...
2. For custom providers, check the jyycode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your jyycode config.
