...
Create `.jyycode/commands/test.md`:
...
You can add custom commands through the JYYCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your JYYCode [config](/docs/config):
...
jyycode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
