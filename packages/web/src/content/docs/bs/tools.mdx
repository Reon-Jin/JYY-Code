...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. JYYCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u JYYCode.
...
:::note
Ovaj alat je dostupan samo kada je `JYYCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `JYYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz JYYCode provajdera ili kada je varijabla `JYYCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju JYYCode:
