---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u JYYCode.
---

import config from "../../../../config.mjs"
export const console = config.console

JYYCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj JYYCode konfiguraciji.
...
u `~/.local/share/jyycode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem JYYCode
config.
...
JYYCode Zen je lista modela koje je obezbedio JYYCode tim koji su bili
testirano i potvrđeno da dobro radi sa JYYCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa JYYCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite jyycode i idite na [jyycode.ai/auth](https://jyycode.ai/auth).
...
Radi kao i svaki drugi provajder u JYYCode i potpuno je opcionalan za korištenje.

---

## JYYCode Go

JYYCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža JYYCode tim i koji su testirani i verificirani da dobro rade s JYYCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `JYYCode Go` i idite na [jyycode.ai/auth](https://jyycode.ai/zen).
...
Da biste koristili Amazon Bedrock s JYYCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete jyycode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `jyycode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u JYYCode.
:::
...
Možete konfigurirati jyycode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi jyycode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju jyycode konfiguraciju.
...
da pohrani token u jyycode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
JYYCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate JYYCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`jyycode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `jyycode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s jyycode:
...
Za korištenje Google Vertex AI s JYYCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće jyycode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem jyycodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [jyycode-helicone-session](https://github.com/H2Shami/jyycode-helicone-session) da automatski prijavite svaki JYYCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki JYYCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati jyycode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati jyycode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati jyycode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za JYYCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/jyycode) za detalje.
:::
...
Da biste koristili Ollama Cloud s JYYCode:
...
3. Kopirajte API ključ za korištenje u JYYCode.
...
6. **Važno**: Prije upotrebe modela oblaka u JYYCode, morate lokalno povući informacije o modelu:
...
JYYCode Zen je lista testiranih i verifikovanih modela koju je obezbedio JYYCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>JYYCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **JYYCode Zen**.

```txt
   /connect
```

3. Unesite svoj JYYCode API ključ.
...
Također možete dodati dodatne modele putem vaše jyycode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše jyycode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Jyycodeom:
...
Također možete prilagoditi modele kroz svoju jyycode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s jyycode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `jyycode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju JYYCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `jyycode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite JYYCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj jyycode konfiguraciji.
