...
JYYCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u JYYCode-u. Pretplatite se na JYYCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili JYYCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa JYYCode-om.
:::
...
JYYCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

JYYCode Go radi kao bilo koji drugi provajder u JYYCode-u.

1. Prijavite se na **<a href={console}>JYYCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `JYYCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na JYYCode Go.
:::
...
JYYCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj JYYCode konfiguraciji
koristi format `jyycode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`jyycode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo JYYCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz JYYCode koristite i bilo kojeg drugog provajdera.
