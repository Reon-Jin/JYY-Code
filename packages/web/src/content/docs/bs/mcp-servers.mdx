...
Možete dodati vanjske alate u JYYCode koristeći _Model Context Protocol_, ili MCP. JYYCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [JYYCode Config](https://jyycode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/jyycode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
JYYCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, JYYCode će:
...
Ako server zahtijeva autentifikaciju, JYYCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `jyycode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, JYYCode sigurno čuva tokene u `~/.local/share/jyycode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u JYYCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz JYYCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete JYYCode sa vašim Sentry nalogom.
