...
Da biste otklonili probleme s JYYCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\jyycode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `jyycode --log-level DEBUG`.

---

## Pohrana

JYYCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/jyycode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\jyycode`
...
JYYCode Desktop pokreće lokalni JYYCode server (`jyycode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `JYYCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/jyycode/jyycode.jsonc` (ili `~/.config/jyycode/jyycode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/jyycode/jyycode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\jyycode\jyycode.jsonc`
...
JYYCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/jyycode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\jyycode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga JYYCode mogao ponovo izgraditi.

1. Potpuno zatvorite JYYCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/jyycode`
- **Linux**: obrišite `~/.cache/jyycode` (ili pokrenite `rm -rf ~/.cache/jyycode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\jyycode`

3. Ponovo pokrenite JYYCode Desktop.

---

### Rješavanje problema sa vezom na serveru

JYYCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `jyycode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `JYYCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `JYYCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, JYYCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa JYYCode karakteristikama.

---

### Obavještenja se ne prikazuju

JYYCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za JYYCode u postavkama vašeg OS-a, i
...
1. Zatvorite JYYCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka JYYCode Desktop aplikacije):

- `jyycode.settings.dat` (URL zadanog servera za desktop)
- `jyycode.global.dat` i `jyycode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `jyycode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\jyycode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. JYYCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\jyycode`

2. Ponovo pokrenite JYYCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti JYYCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
JYYCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
