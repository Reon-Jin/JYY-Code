...
Agent skills omogucavaju JYYCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
JYYCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.jyycode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/jyycode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, JYYCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.jyycode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.jyycode/skills/git-release/SKILL.md` ovako:
...
JYYCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `jyycode.json`:
...
**Za ugradene agente** (u `jyycode.json`):
