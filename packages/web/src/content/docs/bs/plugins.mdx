---
title: Dodaci
description: Napišite vlastite dodatke za proširenje JYYCode.
---

Dodaci vam omogućavaju da proširite JYYCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja JYYCode.
...
- `.jyycode/plugins/` - Dodaci na nivou projekta
- `~/.config/jyycode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/jyycode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/jyycode/jyycode.json`)
2. Konfiguracija projekta (`jyycode.json`)
3. Globalni direktorij dodataka (`~/.config/jyycode/plugins/`)
4. Direktorij dodataka projekta (`.jyycode/plugins/`)
...
JYYCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Jyycode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje JYYCode.
...
Spriječite jyycode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u jyycode:
...
Pomoćnik `tool` kreira prilagođeni alat koji jyycode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za jyycode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://jyycode.ai/docs/sdk) za detalje.
