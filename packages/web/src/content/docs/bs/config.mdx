---
title: Konfiguracija
description: Korištenje JYYCode JSON konfiguracije.
---

Možete konfigurirati JYYCode koristeći JSON konfiguracijski fajl.

---

## Format

JYYCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/jyycode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/jyycode/jyycode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`JYYCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`jyycode.json` u projektu) - postavke specifične za projekat
5. **`.jyycode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`JYYCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.jyycode` i `~/.config/jyycode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/jyycode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu JYYCode konfiguraciju u `~/.config/jyycode/jyycode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/jyycode/tui.json`.
...
Dodajte `jyycode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se JYYCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `JYYCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `JYYCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.jyycode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.jyycode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`jyycode.ai/config.json`**](https://jyycode.ai/config.json).

TUI konfiguracija koristi [**`jyycode.ai/tui.json`**](https://jyycode.ai/tui.json).
...
Koristite `JYYCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `jyycode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `jyycode serve` i `jyycode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš JYYCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `jyycode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj JYYCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, JYYCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/jyycode/agents/` ili `.jyycode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, JYYCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`jyycode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/jyycode/commands/` ili `.jyycode/commands/`. [Saznajte više ovdje](/docs/commands).
...
JYYCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, JYYCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju JYYCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.jyycode/plugins/` ili `~/.config/jyycode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite JYYCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
