---
title: Server
description: Komunicirajte s jyycode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `jyycode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji jyycode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `jyycode.local` |
...
Postavite `JYYCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `jyycode`, ili postavite `JYYCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `jyycode serve` i za `jyycode web`.
...
Kada pokrenete `jyycode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite jyycode server za programsku interakciju sa jyycode-om.
:::

Ova arhitektura omogucava jyycode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `jyycode serve` da startate standalone server. Ako je jyycode TUI vec pokrenut, `jyycode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste JYYCode [IDE](/docs/ide) pluginovi.
...
jyycode server izlaže sljedece API-je.
