...
U JYYCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, JYYCode koristi nasu `jyycode` temu.
...
JYYCode dolazi sa vise ugradenih tema.
...
- Zele da JYYCode odgovara izgledu njihovog terminala
...
JYYCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/jyycode/themes/*.json` ili `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.jyycode/themes/*.json`
4. **Trenutni radni direktorij** - `./.jyycode/themes/*.json`
