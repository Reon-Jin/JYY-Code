...
Postoje dvije vrste agenata u JYYCode: primarni agenti i podagenti.
...
JYYCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
JYYCode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

JYYCode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u JYYCode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `jyycode.json`:
...
- Globalno: `~/.config/jyycode/agents/`
- Po projektu: `.jyycode/agents/`

```markdown title="~/.config/jyycode/agents/review.md"
---
...
Ako temperatura nije navedena, JYYCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu JYYCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj JYYCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [JYYCode Zen](/docs/zen), koristili biste `jyycode/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="jyycode.json" {3-6,9-12}
{
  "$schema": "https://jyycode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="jyycode.json" {3-5,8-10}
{
  "$schema": "https://jyycode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `jyycode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/jyycode).
:::
...
```markdown title="~/.config/jyycode/agents/docs-writer.md"
---
...
```markdown title="~/.config/jyycode/agents/security-auditor.md"
---
