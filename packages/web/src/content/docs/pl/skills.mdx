...
Umiejętności agenta pozwalają jyycode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
jyycode przeszukuje te lokalizacje:

- Project config: `.jyycode/skills/<name>/SKILL.md`
- Global config: `~/.config/jyycode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu jyycode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.jyycode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.jyycode/skills/git-release/SKILL.md` w ten sposób:
...
jyycode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `jyycode.json`:
...
**Dla agentów wbudowanych** (w `jyycode.json`):
