---
title: Serwer
description: Interakcja z serwerem jyycode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `jyycode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient jyycode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `jyycode.local` |
...
Ustaw `JYYCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `jyycode` lub ustaw `JYYCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `jyycode serve`, jak i `jyycode web`.
...
Kiedy uruchomisz `jyycode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera jyycode do programowej interakcji z jyycode.
:::

Ta architektura umożliwia obsługę wielu klientów przez jyycode i programową interakcję z kodem otwartym.

Możesz uruchomić `jyycode serve`, aby uruchomić samodzielny serwer. Jeśli masz
jyycode TUI działa, `jyycode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki jyycode [IDE](/docs/ide).
...
Serwer jyycode udostępnia następujące interfejsy API.
