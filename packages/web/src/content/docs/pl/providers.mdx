---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w jyycode.
---

import config from "../../../../config.mjs"
export const console = config.console

jyycode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji jyycode.
...
w `~/.local/share/jyycode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim jyycode
config.
...
JYYCode Zen to lista modeli dostarczonych przez zespół jyycode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z jyycode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od JYYCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `JYYCode Zen` i przejdź do [jyycode.ai/auth](https://jyycode.ai/zen).
...
Działa jak każdy inny dostawca w jyycode i jest całkowicie opcjonalny w użyciu.

---

## JYYCode Go

JYYCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół jyycode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z jyycode.

1. Uruchom polecenie `/connect` w TUI, wybierz `JYYCode Go` i przejdź do [jyycode.ai/auth](https://jyycode.ai/zen).
...
Aby używać Amazon Bedrock z jyycode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania jyycode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `jyycode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w jyycode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować jyycode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod jyycode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji jyycode.
...
jyycode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować jyycode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`jyycode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem jyycode:
...
Aby używać Google Vertex AI z jyycode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania jyycode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą jyycode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [jyycode-helicone-session](https://github.com/H2Shami/jyycode-helicone-session), aby automatycznie rejestrować każdą konwersację jyycode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja jyycode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować jyycode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować jyycode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować jyycode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem jyycode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/jyycode).
:::
...
Aby korzystać z Ollama Cloud z jyycode:
...
3. Skopiuj klucz API do użycia w jyycode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w jyycode musisz pobrać informacje o modelu lokalnie:
...
JYYCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół jyycode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>JYYCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **JYYCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API jyycode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji jyycode.
...
5. Możesz je także dostosować za pomocą konfiguracji jyycode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z jyycode:
...
Możesz także dostosować modele za pomocą konfiguracji jyycode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z jyycode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `jyycode.json` w katalogu projektu:
...
Pola `limit` pozwalają jyycode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `jyycode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację jyycode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji jyycode.
