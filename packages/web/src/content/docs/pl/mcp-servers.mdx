...
Możesz dodać dodatkowe informacje do jyycode za pomocą _Model Context Protocol_, w skrócie MCP. jyycode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [jyycode Config](https://jyycode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/jyycode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
jyycode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, jyycode:
...
Jeśli serwer wymaga uwierzytelnienia, jyycode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `jyycode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji jyycode bezpieczne przechowa tokeny w `~/.local/share/jyycode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w jyycode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie jyycode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć jyycode z kontem Sentry.
