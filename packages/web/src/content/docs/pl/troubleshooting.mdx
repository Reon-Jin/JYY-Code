...
Aby debugować problemy z jyycode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\jyycode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `jyycode --log-level DEBUG`.

---

## Przechowywanie danych

jyycode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/jyycode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\jyycode`
...
jyycode Desktop uruchamia w tle lokalny serwer jyycode (sidecar `jyycode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `JYYCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/jyycode/jyycode.jsonc` (lub `~/.config/jyycode/jyycode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/jyycode/jyycode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\jyycode\jyycode.jsonc`
...
jyycode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/jyycode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\jyycode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby jyycode mógł ją odbudować.

1. Całkowicie zamknij jyycode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/jyycode`
- **Linux**: usuń `~/.cache/jyycode` (lub uruchom `rm -rf ~/.cache/jyycode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\jyycode`

3. Uruchom ponownie jyycode Desktop.

---

### Napraw problemy z połączeniem z serwerem

jyycode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `jyycode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `JYYCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `JYYCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows jyycode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami jyycode.

---

### Brak powiadomień

jyycode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla jyycode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie jyycode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji jyycode Desktop):

- `jyycode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `jyycode.global.dat` i `jyycode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `jyycode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `jyycode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\jyycode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. jyycode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\jyycode`

2. Uruchom ponownie kod jyycode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to jyycode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
jyycode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
