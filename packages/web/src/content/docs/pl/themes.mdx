...
Dzięki jyycode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie jyycode używa naszego własnego motywu `jyycode`.
...
jyycode ma kilka wbudowanych motywów.
...
- Chcą, aby JYYCode pasował do wyglądu ich terminala
...
jyycode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/jyycode/themes/*.json` lub `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.jyycode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.jyycode/themes/*.json`
