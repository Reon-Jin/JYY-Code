---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć jyycode.
---

Wtyczki umożliwiające wyłączenie jyycode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania jyycode.
...
- `.jyycode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/jyycode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/jyycode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/jyycode/jyycode.json`)
2. Konfiguracja projektu (`jyycode.json`)
3. Globalny katalog wtyczek (`~/.config/jyycode/plugins/`)
4. Katalog wtyczek projektu (`.jyycode/plugins/`)
...
jyycode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z jyycode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do jyycode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej jyycode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj jyycode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do jyycode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod jyycode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://jyycode.ai/docs/sdk).
