---
title: Server
description: Interact with jyycode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `jyycode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an jyycode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `jyycode.local` |
...
Set `JYYCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `jyycode`, or set `JYYCODE_SERVER_USERNAME` to override it. This applies to both `jyycode serve` and `jyycode web`.
...
When you run `jyycode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the jyycode server to interact with jyycode programmatically.
:::

This architecture lets jyycode support multiple clients and allows you to interact with jyycode programmatically.

You can run `jyycode serve` to start a standalone server. If you have the
jyycode TUI running, `jyycode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the JYYCode [IDE](/docs/ide) plugins.
...
The jyycode server exposes the following APIs.
