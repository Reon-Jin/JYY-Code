---
title: Server
description: Interagisci con il server jyycode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `jyycode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client jyycode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `jyycode.local` |
...
Imposta `JYYCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `jyycode`, oppure imposta `JYYCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `jyycode serve` sia per `jyycode web`.
...
Quando esegui `jyycode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server jyycode per interagire con jyycode in modo programmatico.
:::

Questa architettura permette a jyycode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `jyycode serve` per avviare un server standalone. Se la TUI di jyycode e gia in esecuzione, `jyycode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di JYYCode.
...
Il server jyycode espone le seguenti API.
