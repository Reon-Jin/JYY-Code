...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. JYYCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in JYYCode.
...
:::note
Questo strumento e' disponibile solo quando `JYYCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `JYYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider JYYCode o quando la variabile d'ambiente `JYYCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di JYYCode:
