---
title: Plugin
description: Scrivi plugin per estendere JYYCode.
---

I plugin ti permettono di estendere JYYCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di JYYCode.
...
- `.jyycode/plugins/` - plugin a livello progetto
- `~/.config/jyycode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/jyycode/node_modules/`.
...
1. Config globale (`~/.config/jyycode/jyycode.json`)
2. Config di progetto (`jyycode.json`)
3. Directory plugin globale (`~/.config/jyycode/plugins/`)
4. Directory plugin di progetto (`.jyycode/plugins/`)
...
JYYCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di jyycode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere jyycode.
...
:::note
Se usi l'app desktop di JYYCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a jyycode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a jyycode:
...
L'helper `tool` crea uno strumento personalizzato che jyycode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in jyycode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://jyycode.ai/docs/sdk) per i dettagli.
