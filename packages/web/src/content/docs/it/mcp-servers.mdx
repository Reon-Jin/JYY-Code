...
Puoi aggiungere strumenti esterni a JYYCode usando il _Model Context Protocol_, o MCP. JYYCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [JYYCode Config](https://jyycode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/jyycode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
JYYCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, JYYCode:
...
Se il server richiede autenticazione, JYYCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `jyycode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, JYYCode memorizzera' i token in modo sicuro in `~/.local/share/jyycode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in JYYCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione JYYCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare JYYCode al tuo account Sentry.
