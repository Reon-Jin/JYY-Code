...
Le skill degli agenti permettono a JYYCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
JYYCode cerca in queste posizioni:

- Config di progetto: `.jyycode/skills/<name>/SKILL.md`
- Config globale: `~/.config/jyycode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, JYYCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.jyycode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.jyycode/skills/git-release/SKILL.md` cosi':
...
JYYCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `jyycode.json`:
...
**Per agenti integrati** (in `jyycode.json`):
