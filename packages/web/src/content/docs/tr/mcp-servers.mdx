...
_Model Bağlam Protokolü_ veya MCP kullanarak jyycode'a harici araçlar ekleyebilirsiniz. jyycode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [jyycode Config](https://jyycode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/jyycode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
jyycode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde jyycode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa jyycode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `jyycode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından jyycode, belirteçleri `~/.local/share/jyycode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra jyycode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi jyycode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve jyycode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
