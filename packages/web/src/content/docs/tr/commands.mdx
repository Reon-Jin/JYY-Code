...
`.jyycode/commands/test.md` oluştur:
...
jyycode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

jyycode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/jyycode/commands/`
- Proje başına: `.jyycode/commands/`
...
jyycode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
