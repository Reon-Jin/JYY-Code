---
title: Sunucu
description: jyycode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`jyycode serve` komutu, jyycode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `jyycode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `JYYCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `jyycode` değeridir; değiştirmek isterseniz `JYYCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `jyycode serve` hem de `jyycode web` için geçerlidir.
...
`jyycode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
jyycode ile programatik etkileşim için jyycode sunucusunu kullanın.
:::

Bu mimari, jyycode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`jyycode serve` ile bağımsız bir sunucu başlatabilirsiniz. jyycode TUI açıksa, `jyycode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum jyycode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
jyycode sunucusu aşağıdaki API'leri sunar.
