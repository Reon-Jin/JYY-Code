---
title: Eklentiler
description: JYYCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek JYYCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya JYYCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.jyycode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/jyycode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/jyycode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/jyycode/jyycode.json`)
2. Project config (`jyycode.json`)
3. Global eklenti dizini (`~/.config/jyycode/plugins/`)
4. Proje eklenti dizini (`.jyycode/plugins/`)
...
JYYCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir JYYCode SDK istemcisi.
...
JYYCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
JYYCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
JYYCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca JYYCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, JYYCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra JYYCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://jyycode.ai/docs/sdk) bakın.
