...
jyycode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak jyycode kendi `jyycode` temasını kullanır.
...
jyycode birden fazla yerleşik temayla gelir.
...
- jyycode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
jyycode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/jyycode/themes/*.json` veya `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.jyycode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.jyycode/themes/*.json`
