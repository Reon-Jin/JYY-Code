...
Ajan becerileri, jyycode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
jyycode su konumlari tarar:

- Proje konfigurasyonu: `.jyycode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/jyycode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda jyycode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.jyycode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.jyycode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
jyycode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`jyycode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`jyycode.json` icinde):
