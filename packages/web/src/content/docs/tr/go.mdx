...
JYYCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, JYYCode'daki diğer sağlayıcılar gibi çalışır. JYYCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve JYYCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
JYYCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

JYYCode Go, JYYCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>JYYCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `JYYCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye JYYCode Go'ya abone olabilir.
:::
...
JYYCode Go aşağıdaki limitleri içerir:
...
JYYCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `jyycode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `jyycode-go/kimi-k2.6` kullanmalısınız.
...
JYYCode Go'yu şu amaçlarla oluşturduk:
...
4. JYYCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
