...
description: JYYCode, LSP sunucularınızla bütünleşir.
---

JYYCode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

JYYCode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`JYYCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, jyycode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
