---
title: Config
description: Using the JYYCode JSON config.
---

You can configure JYYCode using a JSON config file.

---

## Format

JYYCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="jyycode.jsonc"
{
...
1. **Remote config** (from `.well-known/jyycode`) - organizational defaults
2. **Global config** (`~/.config/jyycode/jyycode.json`) - user preferences
3. **Custom config** (`JYYCODE_CONFIG` env var) - custom overrides
4. **Project config** (`jyycode.json` in project) - project-specific settings
5. **`.jyycode` directories** - agents, commands, plugins
6. **Inline config** (`JYYCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/jyycode/` on macOS) - admin-controlled
...
:::note
The `.jyycode` and `~/.config/jyycode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/jyycode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/jyycode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global JYYCode config in `~/.config/jyycode/jyycode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/jyycode/tui.json`.
...
Add `jyycode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When JYYCode starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `JYYCODE_CONFIG` environment variable.

```bash
export JYYCODE_CONFIG=/path/to/my/custom-config.json
jyycode run "Hello world"
```
...
Specify a custom config directory using the `JYYCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.jyycode` directory, and should
follow the same structure.

```bash
export JYYCODE_CONFIG_DIR=/path/to/my/config-directory
jyycode run "Hello world"
```

The custom directory is loaded after the global config and `.jyycode` directories, so it **can override** their settings.
...
Drop an `jyycode.json` or `jyycode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/jyycode/` |
| Linux    | `/etc/jyycode/`                         |
| Windows  | `%ProgramData%\jyycode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, JYYCode reads managed preferences from the `ai.jyycode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

JYYCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.jyycode.managed.plist`
2. `/Library/Managed Preferences/ai.jyycode.managed.plist`

The plist keys map directly to `jyycode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.jyycode.managed` PayloadType. The JYYCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.jyycode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.jyycode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.jyycode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`jyycode.ai/config.json`**](https://jyycode.ai/config.json).

TUI config uses [**`jyycode.ai/tui.json`**](https://jyycode.ai/tui.json).
...
Use `JYYCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `jyycode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `jyycode serve` and `jyycode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your JYYCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `jyycode.local`. Useful for running multiple instances on the same network.
...
If not specified, JYYCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your JYYCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, JYYCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
JYYCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, JYYCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/jyycode/agents/` or `.jyycode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, JYYCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`jyycode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/jyycode/commands/` or `.jyycode/commands/`. [Learn more here](/docs/commands).
...
JYYCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
JYYCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, jyycode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend JYYCode with custom tools, hooks, and integrations.

Place plugin files in `.jyycode/plugins/` or `~/.config/jyycode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="jyycode.json"
{
  "$schema": "https://jyycode.ai/config.json",
  "plugin": ["jyycode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict JYYCode to only use specific providers rather than disabling them one by one.
