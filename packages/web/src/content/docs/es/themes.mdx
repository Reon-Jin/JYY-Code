...
Con JYYCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, JYYCode usa nuestro propio tema `jyycode`.
...
JYYCode viene con varios temas integrados.
...
- Quiere que JYYCode coincida con la apariencia de su terminal
...
JYYCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/jyycode/themes/*.json` o `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.jyycode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.jyycode/themes/*.json`
