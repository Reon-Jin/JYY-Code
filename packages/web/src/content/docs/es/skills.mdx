...
Las habilidades del agente permiten a JYYCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
JYYCode busca estas ubicaciones:

- Configuración del proyecto: `.jyycode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/jyycode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, JYYCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.jyycode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.jyycode/skills/git-release/SKILL.md` así:
...
JYYCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `jyycode.json`:
...
**Para agentes integrados** (en `jyycode.json`):
