---
title: Complementos
description: Escriba sus propios complementos para extender JYYCode.
---

Los complementos le permiten extender JYYCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de JYYCode.
...
- `.jyycode/plugins/` - Complementos a nivel de proyecto
- `~/.config/jyycode/plugins/` - Complementos globales
...
```json title="jyycode.json"
{
  ...
  "plugin": ["jyycode-helicone-session", "jyycode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/jyycode/node_modules/`.
...
1. Configuración global (`~/.config/jyycode/jyycode.json`)
2. Configuración del proyecto (`jyycode.json`)
3. Directorio global de complementos (`~/.config/jyycode/plugins/`)
4. Directorio de complementos del proyecto (`.jyycode/plugins/`)
...
```json title=".jyycode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
JYYCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".jyycode/plugins/my-plugin.ts"
...
```js title=".jyycode/plugins/example.js"
...
- `client`: Un cliente SDK jyycode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar jyycode.
...
```js title=".jyycode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "jyycode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio JYYCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que jyycode lea archivos `.env`:

```javascript title=".jyycode/plugins/env-protection.js"
...
```javascript title=".jyycode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a jyycode:

```ts title=".jyycode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@jyycode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que jyycode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para jyycode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://jyycode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".jyycode/plugins/compaction.ts"
import type { Plugin } from "@jyycode-ai/plugin"
...
```ts title=".jyycode/plugins/custom-compaction.ts"
import type { Plugin } from "@jyycode-ai/plugin"
