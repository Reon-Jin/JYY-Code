---
title: Servidor
description: Interactuar con el servidor jyycode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `jyycode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente jyycode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `jyycode.local` |
...
Configure `JYYCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `jyycode`, o configure `JYYCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `jyycode serve` como a `jyycode web`.
...
Cuando ejecuta `jyycode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor jyycode para interactuar con jyycode mediante programación.
:::

Esta arquitectura permite que jyycode admita múltiples clientes y le permite interactuar con jyycode mediante programación.

Puede ejecutar `jyycode serve` para iniciar un servidor independiente. Si tienes el
jyycode TUI ejecutándose, `jyycode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos JYYCode [IDE](/docs/ide).
...
El servidor jyycode expone las siguientes API.
