...
description: Cree herramientas que LLM pueda llamar en jyycode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de jyycode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.jyycode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/jyycode/tools/`.
...
```ts title=".jyycode/tools/database.ts" {1}
...
```ts title=".jyycode/tools/math.ts"
import { tool } from "@jyycode-ai/plugin"
...
```ts title=".jyycode/tools/bash.ts"
import { tool } from "@jyycode-ai/plugin"
...
```ts title=".jyycode/tools/project.ts" {8}
import { tool } from "@jyycode-ai/plugin"
...
```python title=".jyycode/tools/add.py"
import sys
...
```ts title=".jyycode/tools/python-add.ts" {10}
import { tool } from "@jyycode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".jyycode/tools/add.py")
