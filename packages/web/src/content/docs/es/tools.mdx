...
Las herramientas permiten que LLM realice acciones en su código base. JYYCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en JYYCode.
...
:::note
Esta herramienta solo está disponible cuando `JYYCODE_EXPERIMENTAL_LSP_TOOL=true` (o `JYYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor JYYCode o cuando la variable de entorno `JYYCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar JYYCode:

```bash
JYYCODE_ENABLE_EXA=1 jyycode
```
