---
title: サーバー
description: HTTP 経由で JYYCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`jyycode serve` コマンドは、JYYCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `jyycode.local` |
...
HTTP 基本認証でサーバーを保護するには、`JYYCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `jyycode` になるか、`JYYCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`jyycode serve` と `jyycode web` の両方に当てはまります。
...
`jyycode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
JYYCode サーバーを使用して、プログラムで JYYCode と対話します。
:::

`jyycode serve` を実行してスタンドアロンサーバーを起動できます。すでに JYYCode TUI を実行している場合でも、`jyycode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、JYYCode [IDE](/docs/ide) プラグインによって使用されます。
