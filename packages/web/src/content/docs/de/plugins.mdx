---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um JYYCode zu erweitern.
---

Mit Plugins können Sie JYYCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von JYYCode zu ändern.
...
- `.jyycode/plugins/` – Plugins auf Projektebene
- `~/.config/jyycode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/jyycode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/jyycode/jyycode.json`)
2. Projektkonfiguration (`jyycode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/jyycode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.jyycode/plugins/`)
...
JYYCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein JYYCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie JYYCode erweitern können.
...
:::note
Wenn Sie die JYYCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass JYYCode `.env`-Dateien liest:
...
Plugins können JYYCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das JYYCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für JYYCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://jyycode.ai/docs/sdk).
