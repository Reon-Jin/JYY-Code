...
Agent Skills erlauben JYYCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
JYYCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft JYYCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.jyycode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.jyycode/skills/git-release/SKILL.md` zum Beispiel so:
...
JYYCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `jyycode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `jyycode.json`):
