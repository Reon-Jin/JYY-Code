...
Wenn JYYCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\jyycode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `jyycode --log-level DEBUG`.

---

## Speicher

jyycode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/jyycode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\jyycode`
...
JYYCode Desktop startet im Hintergrund einen lokalen JYYCode-Server (`jyycode-cli`-Sidecar).
...
- Nur macOS: `JYYCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/jyycode/jyycode.jsonc` (oder `~/.config/jyycode/jyycode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/jyycode/jyycode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\jyycode\jyycode.jsonc`
...
JYYCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/jyycode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\jyycode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit JYYCode ihn neu aufbauen kann.

1. Beende JYYCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/jyycode`
- **Linux**: loesche `~/.cache/jyycode` (oder fuehre aus: `rm -rf ~/.cache/jyycode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\jyycode`

3. Starte JYYCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

JYYCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `jyycode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `JYYCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `JYYCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt JYYCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit JYYCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

JYYCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer JYYCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende JYYCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von JYYCode Desktop):

- `jyycode.settings.dat` (desktop default server URL)
- `jyycode.global.dat` und `jyycode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit JYYCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `jyycode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\jyycode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. jyycode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\jyycode`

2. Starte jyycode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt jyycode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
jyycode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
