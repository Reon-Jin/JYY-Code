---
title: Server
description: Nutze den jyycode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `jyycode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein jyycode-Client nutzen kann.
...
Setze `JYYCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `jyycode`, kann aber mit `JYYCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `jyycode serve` und `jyycode web`.
...
Wenn du `jyycode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den jyycode-Server, um jyycode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `jyycode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `jyycode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die JYYCode-[IDE](/docs/ide)-Plugins.
...
Der jyycode-Server stellt folgende APIs bereit.
