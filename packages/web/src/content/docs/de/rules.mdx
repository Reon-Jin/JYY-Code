---
title: Regeln
description: Lege eigene Anweisungen fuer jyycode fest.
---

Du kannst jyycode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in jyycode den Befehl `/init` aus.
...
So kann jyycode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht jyycode sie zu erweitern.
...
jyycode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/jyycode/AGENTS.md` speichern.
Diese gelten in allen jyycode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt JYYCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/jyycode/AGENTS.md` existiert)
...
Beim Start sucht jyycode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/jyycode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `jyycode.json` oder in `~/.config/jyycode/jyycode.json` angeben.
...
jyycode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit jyycode.json

Empfohlen ist das `instructions`-Feld in der `jyycode.json`:
...
Du kannst jyycode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `jyycode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
