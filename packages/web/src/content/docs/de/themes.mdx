...
Mit JYYCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet JYYCode das `jyycode`-Theme.
...
JYYCode liefert mehrere eingebaute Themes mit.
...
- JYYCode optisch ans Terminal anpassen wollen
...
JYYCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/jyycode/themes/*.json` oder `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.jyycode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.jyycode/themes/*.json`
