...
JYYCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in JYYCode. Du abonnierst JYYCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
JYYCode zu verwenden.
...
JYYCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

JYYCode Go funktioniert wie jeder andere Provider in JYYCode.

1. Du meldest dich bei **<a href={console}>JYYCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `JYYCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann JYYCode Go abonnieren.
:::
...
JYYCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner JYYCode Config verwendet das Format `jyycode-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `jyycode-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben JYYCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit JYYCode zu nutzen.
