...
Erstellen Sie `.jyycode/commands/test.md`:

```md title=".jyycode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die JYYCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem JYYCode [config](/docs/config):

```json title="jyycode.jsonc" {4-12}
{
...
- Global: `~/.config/jyycode/commands/`
- Pro Projekt: `.jyycode/commands/`

```markdown title="~/.config/jyycode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".jyycode/commands/component.md"
---
...
```md title=".jyycode/commands/create-file.md"
---
...
```md title=".jyycode/commands/analyze-coverage.md"
---
...
```md title=".jyycode/commands/review-changes.md"
---
...
```md title=".jyycode/commands/review-component.md"
---
...
JYYCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
