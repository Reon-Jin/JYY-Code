...
Es gibt zwei Arten von Agenten in JYYCode; Primäragenten und Subagenten.
...
JYYCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `jyycode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet JYYCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale JYYCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer JYYCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [JYYCode Zen](/docs/zen) verwenden, würden Sie `jyycode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `jyycode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/jyycode).
:::
