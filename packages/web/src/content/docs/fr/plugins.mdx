---
title: Plugins
description: Écrivez vos propres plugins pour étendre JYYCode.
---

Les plugins vous permettent d'étendre JYYCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de JYYCode.
...
- `.jyycode/plugins/` - Plugins au niveau du projet
- `~/.config/jyycode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/jyycode/node_modules/`.
...
1. Configuration globale (`~/.config/jyycode/jyycode.json`)
2. Configuration du projet (`jyycode.json`)
3. Répertoire global des plugins (`~/.config/jyycode/plugins/`)
4. Répertoire des plugins du projet (`.jyycode/plugins/`)
...
JYYCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK jyycode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre jyycode.
...
:::note
Si vous utilisez l'application de bureau JYYCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez jyycode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à jyycode :
...
L'assistant `tool` crée un outil personnalisé que jyycode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour jyycode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://jyycode.ai/docs/sdk) pour plus de détails.
