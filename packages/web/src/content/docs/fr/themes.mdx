...
Avec JYYCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, JYYCode utilise notre propre thème `jyycode`.
...
JYYCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que JYYCode corresponde à l'apparence de leur terminal
...
JYYCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/jyycode/themes/*.json` ou `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.jyycode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.jyycode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/jyycode/themes
vim ~/.config/jyycode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .jyycode/themes
vim .jyycode/themes/my-theme.json
```
