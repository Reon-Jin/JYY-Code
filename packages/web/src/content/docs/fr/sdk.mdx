---
title: SDK
description: Client JS de type sécurisé pour le serveur jyycode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK jyycode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler jyycode par programme.
...
Créez une instance de jyycode :

```javascript
import { createJyycode } from "@jyycode-ai/sdk"

const { client } = await createJyycode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `jyycode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${jyycode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de jyycode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@jyycode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*jyycode" },
})
