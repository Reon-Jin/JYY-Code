---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en JYYCode.
---

import config from "../../../../config.mjs"
export const console = config.console

JYYCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration JYYCode.
...
en `~/.local/share/jyycode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre JYYCode
configuration.
...
JYYCode Zen est une liste de modèles fournis par l'équipe JYYCode qui ont été
testé et vérifié pour fonctionner correctement avec JYYCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par JYYCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez jyycode et dirigez-vous vers [jyycode.ai/auth](https://jyycode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans JYYCode et son utilisation est totalement facultative.

---

## JYYCode Go

JYYCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe JYYCode qui ont été
testé et vérifié pour fonctionner correctement avec JYYCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `JYYCode Go` et rendez-vous sur [jyycode.ai/auth](https://jyycode.ai/zen).
...
Pour utiliser Amazon Bedrock avec JYYCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de jyycode :
...
Pour une configuration spécifique au projet ou persistante, utilisez `jyycode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans JYYCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer jyycode pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que jyycode fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration jyycode.
...
pour stocker le jeton dans le stockage d'authentification jyycode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
JYYCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller JYYCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `jyycode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `jyycode.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec jyycode :
...
Pour utiliser Google Vertex AI avec JYYCode :
...
Définissez-les lors de l'exécution de jyycode.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via jyycode, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [jyycode-helicone-session](https://github.com/H2Shami/jyycode-helicone-session) pour enregistrer automatiquement chaque conversation JYYCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation JYYCode répertoriée comme une session distincte.
...
Vous pouvez configurer jyycode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer jyycode pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer jyycode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour JYYCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/jyycode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec JYYCode :
...
3. Copiez la clé API à utiliser dans JYYCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans JYYCode, vous devez extraire les informations du modèle localement :
...
JYYCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe JYYCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>JYYCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **JYYCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé JYYCode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration jyycode.
...
5. Vous pouvez également les personnaliser via votre configuration jyycode. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Jyycode :
...
Vous pouvez également personnaliser les modèles via votre configuration jyycode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec jyycode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `jyycode.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à JYYCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `jyycode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration jyycode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration jyycode.
