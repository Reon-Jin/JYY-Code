---
title: TUI
description: Utilisation de l’interface utilisateur du terminal JYYCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

JYYCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de JYYCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le JYYCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à JYYCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `jyycode.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle JYYCode attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `jyycode.default`.
