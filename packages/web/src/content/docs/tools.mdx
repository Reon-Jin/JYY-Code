...
Tools allow the LLM to perform actions in your codebase. JYYCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in JYYCode.
...
:::note
This tool is only available when `JYYCODE_EXPERIMENTAL_LSP_TOOL=true` (or `JYYCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the JYYCode provider or when the `JYYCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching JYYCode:
