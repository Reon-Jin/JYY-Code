...
You can add external tools to JYYCode using the _Model Context Protocol_, or MCP. JYYCode supports both local and remote servers.
...
You can define MCP servers in your [JYYCode Config](https://jyycode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/jyycode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
JYYCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, JYYCode will:
...
If the server requires authentication, JYYCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `jyycode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, JYYCode will store the tokens securely in `~/.local/share/jyycode/mcp-auth.json`.
...
Your MCPs are available as tools in JYYCode, alongside built-in tools. So you can manage them through the JYYCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect JYYCode to your Sentry account.
