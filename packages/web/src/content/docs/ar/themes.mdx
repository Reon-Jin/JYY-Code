...
مع JYYCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم JYYCode سمتنا `jyycode`.
...
يأتي JYYCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق JYYCode مظهر terminal لديهم
...
يدعم JYYCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/jyycode/themes/*.json` أو `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.jyycode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.jyycode/themes/*.json`
