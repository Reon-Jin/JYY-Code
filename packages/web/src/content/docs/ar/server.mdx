---
title: الخادم
description: تفاعل مع خادم jyycode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `jyycode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل jyycode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `jyycode.local` |
...
عيّن `JYYCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `jyycode`، أو عيّن `JYYCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `jyycode serve` و `jyycode web`.
...
عند تشغيل `jyycode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم jyycode للتفاعل مع jyycode برمجيا.
:::

تتيح هذه البنية لـ jyycode دعم عدة عملاء وتمكّنك من التفاعل مع jyycode برمجيا.

يمكنك تشغيل `jyycode serve` لبدء خادم مستقل. إذا كانت واجهة jyycode في terminal (TUI)
قيد التشغيل، فسيبدأ `jyycode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات JYYCode لـ [IDE](/docs/ide).
...
يعرض خادم jyycode واجهات API التالية.
