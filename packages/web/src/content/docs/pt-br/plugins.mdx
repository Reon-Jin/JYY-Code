---
title: Plugins
description: Escreva seus próprios plugins para estender o jyycode.
---

Plugins permitem que você estenda o jyycode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do jyycode.
...
- `.jyycode/plugins/` - Plugins em nível de projeto
- `~/.config/jyycode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/jyycode/node_modules/`.
...
1. Configuração global (`~/.config/jyycode/jyycode.json`)
2. Configuração do projeto (`jyycode.json`)
3. Diretório de plugins global (`~/.config/jyycode/plugins/`)
4. Diretório de plugins do projeto (`.jyycode/plugins/`)
...
O jyycode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do jyycode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o jyycode.
...
:::note
Se você estiver usando o aplicativo desktop jyycode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o jyycode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao jyycode:
...
O helper `tool` cria uma ferramenta personalizada que o jyycode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o jyycode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://jyycode.ai/docs/sdk) para detalhes.
