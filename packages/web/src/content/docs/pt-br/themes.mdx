...
Com o jyycode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o jyycode usa nosso próprio tema `jyycode`.
...
O jyycode vem com vários temas embutidos.
...
- Querem que o jyycode corresponda à aparência do seu terminal
...
O jyycode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/jyycode/themes/*.json` ou `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.jyycode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.jyycode/themes/*.json`
