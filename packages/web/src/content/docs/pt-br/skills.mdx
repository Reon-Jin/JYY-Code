...
As habilidades do agente permitem que o jyycode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O jyycode pesquisa nesses locais:

- Configuração do projeto: `.jyycode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/jyycode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o jyycode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.jyycode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.jyycode/skills/git-release/SKILL.md` assim:
...
O jyycode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `jyycode.json`:
...
**Para agentes embutidos** (em `jyycode.json`):
