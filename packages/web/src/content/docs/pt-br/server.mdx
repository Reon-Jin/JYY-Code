---
title: Servidor
description: Interaja com o servidor jyycode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `jyycode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente jyycode pode usar.
...
```bash
jyycode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `jyycode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
jyycode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `JYYCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `jyycode`, ou defina `JYYCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `jyycode serve` quanto ao `jyycode web`.

```bash
JYYCODE_SERVER_PASSWORD=your-password jyycode serve
```
...
Quando você executa `jyycode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor jyycode para interagir com o jyycode programaticamente.
:::

Essa arquitetura permite que o jyycode suporte múltiplos clientes e permite que você interaja com o jyycode programaticamente.

Você pode executar `jyycode serve` para iniciar um servidor autônomo. Se você tiver o TUI do jyycode em execução, `jyycode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do jyycode [IDE](/docs/ide).
...
O servidor jyycode expõe as seguintes APIs.
