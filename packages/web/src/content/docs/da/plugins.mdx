---
title: Plugins
description: Skriv dine egne plugins for at udvide JYYCode.
---

Plugins giver dig mulighed for at udvide JYYCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre JYYCodes standardadfærd.
...
- `.jyycode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/jyycode/node_modules/`.
...
1. Global konfiguration (`~/.config/jyycode/jyycode.json`)
2. Projektkonfiguration (`jyycode.json`)
3. Global plugin-mappe (`~/.config/jyycode/plugins/`)
4. Projekt plugin bibliotek (`.jyycode/plugins/`)
...
JYYCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En jyycode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide jyycode.
...
:::note
Hvis du bruger JYYCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå jyycode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til jyycode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som jyycode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for jyycode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://jyycode.ai/docs/sdk) for detaljer.
