...
Med JYYCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger JYYCode vårt eget `jyycode`-tema.
...
JYYCode kommer med flere innebygde temaer.
...
- Vil at JYYCode skal matche terminalens utseende
...
JYYCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/jyycode/themes/*.json` eller `$XDG_CONFIG_HOME/jyycode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.jyycode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.jyycode/themes/*.json`
