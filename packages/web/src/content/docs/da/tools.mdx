...
Her er alle de innebygde verktøyene som er tilgængelige i JYYCode.
...
:::note
Dette verktøyet er bare tilgængelig når `JYYCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `JYYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger JYYCode-leverandøren eller når miljøvariabelen `JYYCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
