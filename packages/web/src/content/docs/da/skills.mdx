...
Agentferdigheter lar JYYCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
JYYCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.jyycode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/jyycode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går JYYCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.jyycode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/jyycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.jyycode/skills/git-release/SKILL.md` slik:
...
JYYCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `jyycode.json`:
...
**For innebygde agenter** (i `jyycode.json`):
