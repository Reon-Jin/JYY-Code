---
title: Server
description: Interager med jyycode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `jyycode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en jyycode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `jyycode.local` |
...
Indstil `JYYCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `jyycode`, eller indstil `JYYCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `jyycode serve` og `jyycode web`.
...
Når du kører `jyycode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug jyycode-serveren til at interagere med jyycode programmatisk.
:::

Denne arkitektur lader jyycode understøtte flere klienter og lader dig interagere med jyycode programmatisk.

Du kan køre `jyycode serve` for at starte en selvstændig server. Hvis du har
jyycode TUI kørende, vil `jyycode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af JYYCode [IDE](/docs/ide) plugins.
...
JYYCode-serveren viser følgende API'er.
