...
Du kan legge til eksterne verktøy i JYYCode ved å bruke _Model Context Protocol_ eller MCP. JYYCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [JYYCode Config](https://jyycode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="jyycode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/jyycode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
JYYCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil JYYCode:
...
Hvis serveren krever autentisering, vil JYYCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `jyycode mcp auth <server-name>`.
...
```bash
jyycode mcp auth my-oauth-server
```
...
```bash
jyycode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil JYYCode lagre tokens sikkert i `~/.local/share/jyycode/mcp-auth.json`.
...
jyycode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i JYYCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom JYYCode-konfigurasjonen som alle andre verktøy.
...
```json title="jyycode.json" {11, 14-18}
{
  "$schema": "https://jyycode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
jyycode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble JYYCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver JYYCode-samtale oppført som en separat økt.
