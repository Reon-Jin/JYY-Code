---
title: Tjener
description: Samhandle med JYYCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `jyycode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en JYYCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `jyycode.local` |
...
Sett `JYYCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `jyycode` som standard, eller sett `JYYCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `jyycode serve` og `jyycode web`.
...
Når du kjører `jyycode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk JYYCode-serveren til å samhandle med JYYCode programmatisk.
:::

Denne arkitekturen lar JYYCode støtte flere klienter og lar deg samhandle med JYYCode programmatisk.

Du kan kjøre `jyycode serve` for å starte en frittstående server. Hvis du har
JYYCode TUI kjører, vil `jyycode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av JYYCode [IDE](/docs/ide) plugins.
...
jyycode-serveren viser følgende APIer.
