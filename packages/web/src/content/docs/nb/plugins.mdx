...
description: Skriv dine egne programtillegg for å utvide JYYCode.
---

Plugins lar deg utvide JYYCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til JYYCode.
...
- `.jyycode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/jyycode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/jyycode/jyycode.json`)
2. Prosjektkonfigurasjon (`jyycode.json`)
3. Global plugin-katalog (`~/.config/jyycode/plugins/`)
4. Prosjektpluginkatalog (`.jyycode/plugins/`)
...
JYYCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En JYYCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide JYYCode.
...
:::note
Hvis du bruker JYYCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre JYYCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til JYYCode:
...
`tool`-hjelperen lager et tilpasset verktøy som JYYCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for JYYCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://jyycode.ai/docs/sdk) for detaljer.
