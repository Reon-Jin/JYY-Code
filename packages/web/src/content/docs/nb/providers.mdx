---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i JYYCode.
---

import config from "../../../../config.mjs"
export const console = config.console

JYYCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i JYYCode-konfigurasjonen.
...
i `~/.local/share/jyycode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i JYYCode
konfig.
...
JYYCode Zen er en liste over modeller levert av JYYCode-teamet som har vært
testet og verifisert for å fungere godt med JYYCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med JYYCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `JYYCode Zen` og gå til [jyycode.ai/auth](https://jyycode.ai/zen).
...
Det fungerer som alle andre leverandører i JYYCode og er helt valgfritt å bruke.

---

## JYYCode Go

JYYCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av JYYCode-teamet som har vært
testet og verifisert for å fungere godt med JYYCode.

1. Kjør kommandoen `/connect` i TUI, velg `JYYCode Go`, og gå til [jyycode.ai/auth](https://jyycode.ai/zen).
...
Slik bruker du Amazon Bedrock med JYYCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører JYYCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `jyycode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i jyycode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere jyycode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at JYYCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom JYYCode-konfigurasjonen.
...
for å lagre token i JYYCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
JYYCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse JYYCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`jyycode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med JYYCode:
...
Slik bruker du Google Vertex AI med JYYCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører JYYCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom JYYCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [jyycode-helicone-session](https://github.com/H2Shami/jyycode-helicone-session) plugin for automatisk å logge hver JYYCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver JYYCode-samtale oppført som en separat økt.
...
Du kan konfigurere JYYCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere JYYCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere JYYCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for JYYCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/jyycode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med JYYCode:
...
3. Kopier API-nøkkelen for bruk i JYYCode.
...
6. **Viktig**: Før du bruker skymodeller i JYYCode, må du hente modellinformasjonen lokalt:
...
JYYCode Zen er en liste over testede og verifiserte modeller levert av JYYCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>JYYCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **JYYCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn JYYCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom JYYCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom JYYCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med JYYCode:
...
Du kan også tilpasse modeller gjennom JYYCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med JYYCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `jyycode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar JYYCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `jyycode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk JYYCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i JYYCode-konfigurasjonen.
