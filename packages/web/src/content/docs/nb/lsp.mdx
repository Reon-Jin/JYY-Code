---
title: LSP-servere
description: jyycode integreres med LSP-serverne dine.
---

JYYCode integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

JYYCode kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `JYYCODE_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når jyycode åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i JYYCode-konfigurasjonen.
