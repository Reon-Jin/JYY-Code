...
const lowerToolResultOutput = (part: ToolResultPart): Effect.Effect<OpenAIResponsesFunctionCallOutput, LLMError> =>
  Effect.gen(function* () {
    // Text/json/error results are encoded as a plain string for backward
    // compatibility with existing cassettes and provider expectations.
    if (part.result.type !== "content") return ProviderShared.toolResultText(part)
    return yield* Effect.forEach(part.result.value as ToolResultContentPart[], (item) =>
      lowerToolResultContentItem(item),
    )
  })

const lowerMessages = (request: LLMRequest): Effect.Effect<OpenAIResponsesInputItem[], LLMError> =>
  Effect.gen(function* () {
