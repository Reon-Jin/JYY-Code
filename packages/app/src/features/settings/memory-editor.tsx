...
export function MemoryEditor(props: {
  ...
  const parsed = createMemo(() => {
    ...
    const list = keywords().split(/[,，、]/u).map((item) => item.normalize("NFKC").trim()).filter(Boolean)
  ...
  return (
    <Dialog
      ...
      footer={
        <>
          <Button variant="secondary" disabled={saving()} onClick={props.onClose}>{tr("github.cancel")}</Button>
          <Button disabled={!parsed().value} loading={saving()} loadingLabel={tr("settings.saving")} onClick={() => void save()}>{tr("github.save")}</Button>
        </>
      }
    >
      <div class="memory-editor__fields">
        <label>
          ...
          <input aria-label={tr("settings.memory-importance")} type="number" min="1" max="10" step="1" value={importance()} onInput={(event) => setImportance(event.currentTarget.value)} />
        </label>
        <label>
          <span>{tr("settings.memory-keywords")}</span>
          <input aria-label={tr("settings.memory-keywords")} value={keywords()} onInput={(event) => setKeywords(event.currentTarget.value)} />
        </label>
        <label>
          <span>{tr("settings.memory-content")}</span>
          <input aria-label={tr("settings.memory-content")} value={content()} onInput={(event) => setContent(event.currentTarget.value)} />
        </label>
      </div>
