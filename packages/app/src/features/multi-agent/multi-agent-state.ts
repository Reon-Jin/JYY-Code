...
function numeric(value: number | string) { return Number(value) || 0 }
...
export function projectAgentClusterState(state: SessionAgentClusterResponse): MultiAgentSnapshot {
  const ordered = [...state.tasks].sort((a, b) => numeric(a.step) - numeric(b.step) || numeric(a.time_created) - numeric(b.time_created) || a.id.localeCompare(b.id))
  const tasks = ordered.map((item): MultiAgentTaskView => {
    const presentation = taskStatusPresentation[item.status]
    return {
      key: item.id, id: item.id, step: numeric(item.step), role: item.role,
      skillName: roleCapability(item.role).skill, skillNames: [...roleCapability(item.role).skills], capabilitySummary: roleCapability(item.role).summary,
      title: item.title, model: item.model, status: item.status, tone: presentation.tone, statusLabel: tr(presentation.labelKey),
      ...(item.child_session_id ? { childSessionID: item.child_session_id } : {}), dependencies: [...item.dependencies], acceptanceCriteria: [...item.acceptance_criteria],
      artifactPaths: [...item.artifact_paths], ...(item.result_summary ? { resultSummary: item.result_summary } : {}), reviewIssues: [...item.review_issues],
      ...(item.last_event ? { lastEvent: item.last_event } : {}), reviewRound: numeric(item.review_round), elapsedMs: Math.max(0, Date.now() - numeric(item.time_created)),
    }
  })
  ...
  const current = steps.find((step) => step.tone === "running" || step.tone === "review") ?? steps.find((step) => step.tone === "queued")
  return { steps, tasks, totalAgents: tasks.length, runningAgents: tasks.filter((t) => t.tone === "running" || t.tone === "review").length,
    doneAgents: tasks.filter((t) => t.tone === "done").length, failedAgents: tasks.filter((t) => t.tone === "failed").length,
    interruptedAgents: tasks.filter((t) => t.tone === "interrupted").length, totalSteps: steps.length, currentStep: current?.index ?? steps.at(-1)?.index ?? 0,
