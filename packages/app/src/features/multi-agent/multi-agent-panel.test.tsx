...
describe("MultiAgentPanelView plan and task interactions", () => {
  it("shows status, goal, progress, step counts, and agent counts in summary order", () => {
    ...
    expect(screen.getByText("Dispatching")).toBeVisible()
    expect(screen.getByText("Deliver the complete Desktop workflow")).toBeVisible()
  ...
  it("renders chronological semantic steps with readable tones and full task disclosure", async () => {
    ...
    expect(screen.getAllByRole("list", { name: /Step/ })).toHaveLength(5)
    ...
    expect(within(disclosure).getByText("Coder")).toBeVisible()
    expect(within(disclosure).getByText("test/coder")).toBeVisible()
    expect(within(disclosure).getAllByText("Revising")[0]).toBeVisible()
  ...
  it("opens only tasks with child Sessions and highlights the selected child", async () => {
    ...
    const open = screen.getByRole("button", { name: "打开子 Agent：Implement the panel" })
    expect(open.closest("li")).toHaveAttribute("data-selected", "true")
    expect(screen.queryByRole("button", { name: "打开子 Agent：Local task" })).not.toBeInTheDocument()
