...
describe("projectAgentClusterState", () => {
  it("projects an empty session task graph", () => {
    expect(projectAgentClusterState({ tasks: [] })).toMatchObject({
      tasks: [],
      steps: [],
      totalAgents: 0,
      currentStep: 0,
  ...
  it("groups durable session steps into completed, active, review, queued, and interrupted waves", () => {
    ...
    expect(snapshot.tasks.find((item) => item.id === "stopped")).toMatchObject({
      tone: "interrupted",
      reviewIssues: ["User redirected this worker"],
    })
