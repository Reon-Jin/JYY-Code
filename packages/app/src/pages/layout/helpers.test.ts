...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("jyycode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("jyycode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("jyycode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("jyycode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("jyycode://open-project")).toBeUndefined()
    expect(parseDeepLink("jyycode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "jyycode://open-project?directory=/a",
      "jyycode://other?directory=/b",
      "jyycode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("jyycode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("jyycode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("jyycode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("jyycode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "jyycode://new-session?directory=/a",
      "jyycode://open-project?directory=/b",
      "jyycode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __JYYCODE__: {
        deepLinks: ["jyycode://open-project?directory=/a"],
      },
    } as unknown as Window & { __JYYCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["jyycode://open-project?directory=/a"])
