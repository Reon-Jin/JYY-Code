import type { Config, JyycodeClient, Path, Project, ProviderAuthResponse, Todo } from "@jyycode-ai/sdk/v2/client"
...
export const loadMcpQuery = (directory: string, sdk: JyycodeClient) =>
  queryOptions({
    queryKey: [directory, "mcp"] as const,
    queryFn: () => sdk.mcp.status().then((r) => r.data ?? {}),
  })

export const loadLspQuery = (directory: string, sdk: JyycodeClient) =>
  queryOptions({
    queryKey: [directory, "lsp"] as const,
    queryFn: () => sdk.lsp.status().then((r) => r.data ?? []),
  })

function makeQueryOptionsApi(globalSDK: () => JyycodeClient, sdkFor: (dir: PathKey) => JyycodeClient) {
  return {
...
function createGlobalSync() {
  ...
  const sdkCache = new Map<string, JyycodeClient>()
