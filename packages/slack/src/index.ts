...
import { createJyycode, type ToolPart } from "@jyycode-ai/sdk"
...
console.log("🚀 Starting jyycode server...")
const jyycode = await createJyycode({
  port: 0,
})
console.log("✅ Jyycode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await jyycode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new jyycode session...")
    const { client, server } = jyycode
    ...
    console.log("✅ Created jyycode session:", createResult.data.id)
  ...
  console.log("📝 Sending to jyycode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Jyycode response:", JSON.stringify(result, null, 2))
