      ...
      "catalog.transform": Effect.fn(function* (evt) {
        for (const item of evt.data) {
          ...
          evt.provider.update(item.provider.id, (provider) => {
            provider.options.headers["HTTP-Referer"] = "https://jyycode.ai/"
            provider.options.headers["X-Title"] = "jyycode"
            provider.options.headers["X-BILLING-INVOKE-ORIGIN"] ??= "JYYCode"
          })
        }
      }),
    }
