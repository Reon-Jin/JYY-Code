...
export const JyycodePlugin = PluginV2.define({
  id: PluginV2.ID.make("jyycode"),
  effect: Effect.gen(function* () {
    let hasKey = false
    return {
      "catalog.transform": Effect.fn(function* (evt) {
        const item = evt.data.find((record) => record.provider.id === ProviderV2.ID.jyycode)
        if (!item) return
        hasKey = Boolean(
          process.env.JYYCODE_API_KEY ||
