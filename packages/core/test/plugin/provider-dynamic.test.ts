...
import { AISDK } from "@jyycode-ai/core/aisdk"
import { ModelV2 } from "@jyycode-ai/core/model"
import { PluginV2 } from "@jyycode-ai/core/plugin"
import { DynamicProviderPlugin } from "@jyycode-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "jyycode-provider-dynamic-"))
