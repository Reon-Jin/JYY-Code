...
import { Project } from "@jyycode-ai/core/project"
import { AbsolutePath } from "@jyycode-ai/core/schema"
...
describe("ProjectV2.resolve", () => {
  ...
  it.live("returns previous cached id from common dir", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => Bun.write(path.join(tmp.path, ".git", "jyycode"), "old-id"))
      const project = yield* Project.Service
  ...
  it.live("does not write the cache while resolving", () =>
    Effect.gen(function* () {
      ...
      expect(yield* Effect.promise(() => Bun.file(path.join(tmp.path, ".git", "jyycode")).exists())).toBe(false)
    }),
  )
