...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@jyycode/acme")).toBe("@jyycode/acme")
    expect(Npm.sanitize("@jyycode/acme@1.0.0")).toBe("@jyycode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/jyycode/acme.git"
    const expected = win ? "acme@git+https_//github.com/jyycode/acme.git" : spec
