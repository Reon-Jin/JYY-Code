  ...
  return (
    <button class="task-row" onClick={props.onOpen}>
      <span class="task-row__body">
        <span class="task-row__meta"><span>{taskProject(props.task)}</span><Status status={props.task.status} /></span>
...
export function Status(props: { status: RemoteTask["status"] }) {
  const label = () => ({ running: "进行中", waiting: "待处理", completed: "已完成", failed: "失败", idle: "空闲" })[props.status]
