  ...
  return (
    <section class="page inbox-page">
      ...
      <div class="filter-strip" role="tablist">
        {filters.map((item) => (
          <button classList={{ "is-active": filter() === item.id }} onClick={() => setFilter(item.id)}>
            {item.label}
          </button>
        ))}
      </div>
      <Show when={items().length > 0} fallback={<p class="empty-state">当前筛选下没有待处理事项。</p>}>
        <div class="inbox-list">
          <For each={items()}>
            {(item) => {
              const task = () => taskFor(item.taskID)
              return (
                <button class="inbox-row" onClick={() => task() && props.onOpenTask(task()!)}>
                  <span class={`inbox-marker inbox-marker--${item.kind}`} />
                  <span>
...
function kindLabel(kind: InboxKind) {
  return { question: "需要回答", permission: "等待批准", failed: "需要重试", completed: "已完成" }[kind]
}
