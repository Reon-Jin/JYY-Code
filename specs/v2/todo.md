...
The jyycode server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "jyycode" instance like in that post i showed
- jyycode instance has stuff like `jyycode.session.prompt()` or
  `jyycode.tool.register({...})`
