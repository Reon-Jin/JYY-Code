...
| 桌面控制     | 用户明确要求后才观察屏幕并操控鼠标键盘；可选 Jev 视觉动作选择              |
...
在 Desktop 应用中，**单智能体根会话**可以观察和操控 Windows 或 macOS 桌面。请选择支持图像输入的模型，并在当前消息中明确要求使用电脑控制，例如：“使用电脑控制打开画图，画一幅画并保存。”普通任务不会启用 `computer` 工具；TUI、多智能体模式和子会话也不能使用。操作受 `computer` 权限控制。

- **未激活 Jev API 密钥时**，使用原有电脑控制模式。Agent 可以查看屏幕截图和带编号的无障碍控件，再移动或点击鼠标、滚动、输入文本、按键、拖动、等待窗口出现，或一次批量执行最多 12 个可预期步骤。每次操作都会返回新截图。
- **激活 Jev API 密钥后**，Desktop 自动切换到视觉动作选择模式。在 **设置 → 高级设置 → Jev API 电脑操控** 中输入密钥。Agent 可观察画面或发起 `choose`：本地无障碍信息和视觉检测生成候选，Jev 选择动作种类，并在需要时选择截图坐标。此时不能直接使用旧版输入动作；目标不明确或画面变化时会停止操作并返回新截图，不会回退到旧版。停用密钥即可恢复原有模式。
- **要定位图标等图形控件**，需在本机配置 OmniParser 的 `icon_detect_v3/model.pt` 权重，并准备 Python、PyTorch、torchvision、NumPy 和 Pillow。程序会识别标准 Hugging Face 缓存中的已测试权重；如果权重放在别处，请将 `JYYCODE_COMPUTER_VISION_MODEL` 设为其绝对路径。EasyOCR 是可选组件。没有可用的本地视觉检测器时，Jev 仍可能选择名称明确的无障碍控件，但可能无法确定图形目标。

Jev 接收结构化候选数据，不直接接收原始屏幕截图。在 macOS 上，还需授予桌面辅助程序“辅助功能”和“屏幕录制”权限。平台细节、模型准备和当前限制见 [Desktop 电脑控制指南](packages/desktop/README.md#computer-control)。
