...
升级后的会话系统将本地历史记录视为持久化应用状态，而不是 TUI 的临时缓存：

- 正常退出 TUI 不会删除当前会话；只有用户显式执行删除操作时才会移除会话。
- 打开 `/sessions` 时，会直接从当前 SQLite 数据库读取最多 100 个根会话，不再受旧版 30 天缓存范围限制；搜索最多返回 30 个匹配的根会话。
- 数据库查询尚未完成或暂时失败时，会话切换器会回退到已同步的内存数据，并保留当前会话和本地固定会话。
- 会话和用户消息能够跨正常关闭、SQLite 连接关闭、进程重启以及打包二进制重启继续存在。
- 会话、消息、消息片段、项目、Todo 和事件日志共用 Effect 作用域管理的 SQLite 生命周期；事件只在事务成功提交后发布，回滚的数据不会进入事件总线。
- 类型化迁移只执行一次，并兼容已有的 Drizzle 迁移日志。Windows 持久化路径会统一规范化，避免路径分隔符或盘符大小写差异隐藏有效会话。
