...
import { createJyycodeClient } from "@jyycode-ai/sdk"
...
const { client, server } = createJyycode()
...
try {
  ...
  console.log("jyycode session", session.id)
  if (shareId) {
...
function createJyycode() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`jyycode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createJyycodeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/jyycode|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/jyycode` or `/oc`")
  }
}
...
async function assertJyycodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to jyycode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.jyycode.ai" : "https://jyycode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.jyycode.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("jyycode-github-action")
    response = await fetch("https://api.jyycode.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/jyycode" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/jyycode") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to jyycode...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "jyycode-agent[bot]"`
  await $`git config --global user.email "jyycode-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `jyycode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/jyycode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[jyycode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
